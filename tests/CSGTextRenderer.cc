 ...
 */
void CSGTextRenderer::process(string &target, const string &src,
                              OpenSCADOperator op) {
...
void CSGTextRenderer::applyToChildren(const AbstractNode &node,
                                      OpenSCADOperator op) {
	...
	if (this->visitedchildren[node.index()].size() > 0) {
		...
		bool first = true;
		for (ChildList::const_iterator iter =
		         this->visitedchildren[node.index()].begin();
		     iter != this->visitedchildren[node.index()].end(); iter++) {
			...
			if (first) {
				N += "(" + this->cache[*chnode];
				// 				if (N.dim != 0) first = false; // FIXME: when
				// can this happen?
				first = false;
			} else {
...
Response CSGTextRenderer::visit(State &state,
                                const AbstractIntersectionNode &node) {
	if (state.isPrefix() && isCached(node)) return Response::PruneTraversal;
	if (state.isPostfix()) {
		if (!isCached(node))
			applyToChildren(node, OpenSCADOperator::INTERSECTION);
		addToParent(state, node);
	}
	return Response::ContinueTraversal;
}
...
Response CSGTextRenderer::visit(State &state, const AbstractPolyNode &node) {
	...
	if (state.isPostfix()) {
		if (!isCached(node)) {
			...
			// FIXME: Will generate one single Nef polyhedron (no csg ops
			// necessary)
...
/*!
    ...
    Call this for _every_ node which affects output during the postfix
   traversal.
*/
void CSGTextRenderer::addToParent(const State &state,
