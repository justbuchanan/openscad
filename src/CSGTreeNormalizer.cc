...
/*!
        NB! for e.g. empty intersections, this can normalize a tree to nothing
   and return nullptr.
*/
shared_ptr<CSGNode> CSGTreeNormalizer::normalize(
    const shared_ptr<CSGNode> &root) {
    this->aborted = false;
    ...
    while (1) {
        ...
        if (this->nodecount > this->limit) {
            PRINTB(
                "WARNING: Normalized tree is growing past %d elements. "
...
/*!
        After aborting, a subtree might have become invalidated (nullptr child
   node) since terms can be instantiated multiple times. This will search for
   nullptr children an recursively repair the corresponding subtree.
 */
...
static bool hasRightNonLeaf(shared_ptr<CSGNode> node) {
    shared_ptr<CSGOperation> op = dynamic_pointer_cast<CSGOperation>(node);
    return op->right() &&
           (dynamic_pointer_cast<CSGLeaf>(op->right()) == nullptr);
}
...
shared_ptr<CSGNode> CSGTreeNormalizer::normalizePass(shared_ptr<CSGNode> node) {
    ...
    do {
        ...
        if (shared_ptr<CSGOperation> op =
                dynamic_pointer_cast<CSGOperation>(node)) {
...
shared_ptr<CSGNode> CSGTreeNormalizer::collapse_null_terms(
    const shared_ptr<CSGNode> &node) {
    shared_ptr<CSGOperation> op = dynamic_pointer_cast<CSGOperation>(node);
    if (op) {
        if (!op->right()) {
...
bool CSGTreeNormalizer::match_and_replace(shared_ptr<CSGNode> &node) {
    ...
    shared_ptr<CSGOperation> rightop =
        dynamic_pointer_cast<CSGOperation>(op->right());
    if (rightop) {
        ...
        // 2.  x * (y + z) -> (x * y) + (x * z)
            ...
            node = CSGOperation::createCSGNode(
                ...
                CSGOperation::createCSGNode(OpenSCADOperator::INTERSECTION, x,
                                            y),
                CSGOperation::createCSGNode(OpenSCADOperator::INTERSECTION, x,
                                            z));
            return true;
        ...
        // 3.  x - (y * z) -> (x - y) + (x - z)
            ...
            node = CSGOperation::createCSGNode(
                OpenSCADOperator::UNION,
                CSGOperation::createCSGNode(OpenSCADOperator::DIFFERENCE, x, y),
                CSGOperation::createCSGNode(OpenSCADOperator::DIFFERENCE, x,
                                            z));
            return true;
        }
        // 4.  x * (y * z) -> (x * y) * z
    ...
    shared_ptr<CSGOperation> leftop =
        dynamic_pointer_cast<CSGOperation>(op->left());
    if (leftop) {
        ...
        // 8. (x + y) - z  -> (x - z) + (y - z)
            ...
            node = CSGOperation::createCSGNode(
                ...
                CSGOperation::createCSGNode(OpenSCADOperator::DIFFERENCE, y,
                                            z));
            return true;
        }
        // 9. (x + y) * z  -> (x * z) + (y * z)
            ...
            node = CSGOperation::createCSGNode(
                ...
                CSGOperation::createCSGNode(OpenSCADOperator::INTERSECTION, x,
                                            z),
                CSGOperation::createCSGNode(OpenSCADOperator::INTERSECTION, y,
                                            z));
            return true;
        }
    }
