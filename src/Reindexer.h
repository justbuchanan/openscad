...
/*!
  ...
  The latter is necessary for VBO's or for unifying texture coordinate indices
  to multiple texture coordinate arrays.
*/
template <typename T>
class Reindexer {
public:
	...
	int lookup(const T &val) {
		typename std::unordered_map<T, int>::const_iterator iter =
		    this->map.find(val);
