...
const AbstractFunction *ModuleContext::findLocalFunction(
    const std::string &name) const {
	if (this->functions_p &&
	    this->functions_p->find(name) != this->functions_p->end()) {
		auto f = this->functions_p->find(name)->second;
		if (!f->is_enabled()) {
			PRINTB(
			    "WARNING: Experimental builtin function '%s' is not enabled.",
			    name);
			return nullptr;
		}
		return f;
...
const AbstractModule *ModuleContext::findLocalModule(
    const std::string &name) const {
	if (this->modules_p &&
	    this->modules_p->find(name) != this->modules_p->end()) {
		auto m = this->modules_p->find(name)->second;
		if (!m->is_enabled()) {
			PRINTB("WARNING: Experimental builtin module '%s' is not enabled.",
			       name);
			return nullptr;
		}
		...
		if (!replacement.empty()) {
			PRINT_DEPRECATION(
			    "The %s() module will be removed in future releases. Use %s "
			    "instead.",
			    name % replacement);
		}
		return m;
...
#ifdef DEBUG
std::string ModuleContext::dump(const AbstractModule *mod,
                                const ModuleInstantiation *inst) {
	std::stringstream s;
	if (inst) {
		s << boost::format("ModuleContext %p (%p) for %s inst (%p) ") % this %
		         this->parent % inst->name() % inst;
	} else {
...
FileContext::FileContext(const Context *parent)
    : ModuleContext(parent), usedlibs_p(nullptr) {}
...
ValuePtr FileContext::evaluate_function(const std::string &name,
	...
	for (const auto &m : *this->usedlibs_p) {
		...
		if (usedmod && usedmod->scope.functions.find(name) !=
		                   usedmod->scope.functions.end())
...
AbstractNode *FileContext::instantiate_module(const ModuleInstantiation &inst,
	...
	for (const auto &m : *this->usedlibs_p) {
		...
		// usedmod is nullptr if the library wasn't be compiled (error or
		// file-not-found)
		if (usedmod && usedmod->scope.modules.find(inst.name()) !=
		                   usedmod->scope.modules.end()) {
			FileContext ctx(this->parent);
			ctx.initializeModule(*usedmod);
			// FIXME: Set document path
#ifdef DEBUG
...
#endif
			return usedmod->scope.modules[inst.name()]->instantiate(&ctx, &inst,
			                                                        evalctx);
		}
	}
