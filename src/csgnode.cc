...
/*!
        ...
        A CSGProduct is a vector of intersections and a vector of subtractions,
   used for CSG rendering.
*/

shared_ptr<CSGNode> CSGOperation::createCSGNode(OpenSCADOperator type,
                                                shared_ptr<CSGNode> left,
                                                shared_ptr<CSGNode> right) {
    // In case we're creating a CSG terms from a pruned tree, left/right can be
    // nullptr
    if (!right) {
...
CSGLeaf::CSGLeaf(const shared_ptr<const Geometry> &geom,
                 const Transform3d &matrix, const Color4f &color,
...
CSGOperation::CSGOperation(OpenSCADOperator type, shared_ptr<CSGNode> left,
                           shared_ptr<CSGNode> right)
    : type(type) {
...
std::string CSGLeaf::dump() { return this->label; }
...
void CSGProducts::import(shared_ptr<CSGNode> csgnode, OpenSCADOperator type,
                         CSGNode::Flag flags) {
