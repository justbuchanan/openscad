...
private:
    Value lookup_radius(const Context &ctx, const std::string &radius_var,
                        const std::string &diameter_var) const;
};
...
public:
    ...
    virtual std::string name() const {
        switch (this->type) {
            ...
            default:
                assert(false &&
                       "PrimitiveNode::name(): Unknown primitive type");
                return "unknown";
        }
    }
 ...
 * @return radius value of type Value::ValueType::NUMBER or
 * Value::ValueType::UNDEFINED if both variables are invalid or not set.
 */
Value PrimitiveModule::lookup_radius(const Context &ctx,
    ...
    if (d->type() == Value::ValueType::NUMBER) {
        if (r_defined) {
            PRINTB(
                "WARNING: Ignoring radius variable '%s' as diameter '%s' is "
                "defined too.",
                radius_var % diameter_var);
        }
...
AbstractNode *PrimitiveModule::instantiate(const Context *ctx,
                                           const ModuleInstantiation *inst,
                                           EvalContext *evalctx) const {
    ...
    switch (this->type) {
        ...
        case primitive_type_e::SPHERE:
            ...
            break;
        case primitive_type_e::CYLINDER:
            args += Assignment("h"), Assignment("r1"), Assignment("r2"),
                Assignment("center");
            break;
        case primitive_type_e::POLYHEDRON:
            args += Assignment("points"), Assignment("faces"),
        ...
        case primitive_type_e::CIRCLE:
            ...
            break;
        case primitive_type_e::POLYGON:
            args += Assignment("points"), Assignment("paths"),
                Assignment("convexity");
            break;
        default:
            assert(false &&
                   "PrimitiveModule::instantiate(): Unknown node type");
    }
    ...
    switch (this->type) {
        ...
        case primitive_type_e::POLYHEDRON: {
            ...
            if (node->faces->type() == Value::ValueType::UNDEFINED) {
                ...
                if (node->faces->type() != Value::ValueType::UNDEFINED) {
                    printDeprecation(
                        "polyhedron(triangles=[]) will be removed in future "
...
/*!
        ...
        May return an empty Geometry creation failed, but will not return
   nullptr.
*/
    ...
    switch (this->type) {
        case primitive_type_e::CUBE: {
            ...
            if (this->x > 0 && this->y > 0 && this->z > 0 &&
                !std::isinf(this->x) > 0 && !std::isinf(this->y) > 0 &&
                !std::isinf(this->z) > 0) {
        ...
        case primitive_type_e::SPHERE: {
            ...
            if (this->r1 > 0 && !std::isinf(this->r1)) {
                ...
                // Uncomment the following three lines to enable experimental
                // sphere tesselation
                //		if (rings % 2 == 0) rings++; // To ensure that the
                //middle ring is at phi == 0 degrees

                auto ring = new ring_s[rings];

                //		double offset = 0.5 * ((fragments / 2) % 2);
                for (int i = 0; i < rings; i++) {
                    //			double phi = (M_PI * (i + offset)) /
                    //(fragments/2);
                ...
                p->append_poly();
                for (int i = 0; i < fragments; i++)
                    p->append_vertex(ring[0].points[i].x, ring[0].points[i].y,
                                     ring[0].z);
                ...
                for (int i = 0; i < rings - 1; i++) {
                    ...
                    while (r1i < fragments || r2i < fragments) {
                        ...
                        sphere_next_r1:
                            ...
                            p->insert_vertex(r1->points[r1i].x,
                                             r1->points[r1i].y, r1->z);
                            p->insert_vertex(r1->points[r1j].x,
                                             r1->points[r1j].y, r1->z);
                            p->insert_vertex(r2->points[r2i % fragments].x,
                                             r2->points[r2i % fragments].y,
                                             r2->z);
                            r1i++;
                        } else {
                        sphere_next_r2:
                            ...
                            p->append_vertex(r2->points[r2i].x,
                                             r2->points[r2i].y, r2->z);
                            p->append_vertex(r2->points[r2j].x,
                                             r2->points[r2j].y, r2->z);
                            p->append_vertex(r1->points[r1i % fragments].x,
                                             r1->points[r1i % fragments].y,
                                             r1->z);
                            r2i++;
                        }
                    }
        ...
        case primitive_type_e::CYLINDER: {
            auto p = new PolySet(3, true);
            g = p;
            if (this->h > 0 && !std::isinf(this->h) && this->r1 >= 0 &&
                this->r2 >= 0 && (this->r1 > 0 || this->r2 > 0) &&
                !std::isinf(this->r1) && !std::isinf(this->r2)) {
                auto fragments =
                    Calc::get_fragments_from_r(std::fmax(this->r1, this->r2),
            ...
            for (size_t i = 0; i < this->faces->toVector().size(); i++) {
                ...
                for (size_t j = 0; j < vec.size(); j++) {
                    ...
                    if (pt < this->points->toVector().size()) {
                        double px, py, pz;
                        if (!this->points->toVector()[pt]->getVec3(px, py,
                                                                   pz) ||
                            std::isinf(px) || std::isinf(py) ||
                            std::isinf(pz)) {
                            PRINTB(
                                "ERROR: Unable to convert point at index %d to "
                                "a vec3 of numbers",
                                j);
                            return p;
                        }
        ...
        case primitive_type_e::SQUARE: {
            ...
            g = p;
            if (this->x > 0 && this->y > 0 && !std::isinf(this->x) &&
                !std::isinf(this->y)) {
                Vector2d v1(0, 0);
        ...
        case primitive_type_e::CIRCLE: {
            ...
            g = p;
            if (this->r1 > 0 && !std::isinf(this->r1)) {
                auto fragments = Calc::get_fragments_from_r(this->r1, this->fn,
                                                            this->fs, this->fa);
        ...
        case primitive_type_e::POLYGON: {
            ...
            for (unsigned int i = 0; i < vec.size(); i++) {
                ...
                if (!val.getVec2(x, y) || std::isinf(x) || std::isinf(y)) {
                    PRINTB(
                        "ERROR: Unable to convert point %s at index %d to a "
                        "vec2 of numbers",
                        val.toString() % i);
                    return p;
                }
                ...
                for (const auto &polygon : this->paths->toVector()) {
                    ...
                    for (const auto &index : polygon->toVector()) {
                        ...
                        if (idx < outline.vertices.size()) {
                            curroutline.vertices.push_back(
                                outline.vertices[idx]);
                        }
...
void register_builtin_primitives() {
    ...
    Builtins::init("polyhedron",
                   new PrimitiveModule(primitive_type_e::POLYHEDRON));
