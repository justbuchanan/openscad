...
public:
	...
	virtual ValuePtr evaluate(const class Context *ctx,
	                          const class EvalContext *evalctx) const = 0;
...
public:
	typedef ValuePtr (*eval_func_t)(const Context *ctx,
...
public:
	...
	UserFunction(const char *name, AssignmentList &definition_arguments,
	...
	static UserFunction *create(const char *name,
	                            AssignmentList &definition_arguments,
	                            shared_ptr<Expression> expr,
	                            const Location &loc);
};
