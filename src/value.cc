...
bool Value::getVec2(double &x, double &y, bool ignoreInfinite) const
{
	...
	double rx, ry;
	bool valid = ignoreInfinite ? v[0]->getFiniteDouble(rx) && v[1]->getFiniteDouble(ry)
	...
	public:                                                                                        \
		...
		{                                                                                            \
			return false;                                                                              \
                                                                                                 ...
                                                                                                 \
		bool operator()(const bool &op1, const bool &op2) const { return op1 op op2; }               \
                                                                                                 \
		bool operator()(const bool &op1, const double &op2) const { return op1 op op2; }             \
                                                                                                 \
		bool operator()(const double &op1, const bool &op2) const { return op1 op op2; }             \
                                                                                                 \
		bool operator()(const double &op1, const double &op2) const { return op1 op op2; }           \
                                                                                                 \
		bool operator()(const std::string &op1, const std::string &op2) const { return op1 op op2; } \
	}
 ...
 * If the string is multi-byte unicode then the index will offset to the character (2 or 4 byte) and
 * not to the byte. A 'normal' string with byte chars are a subset of unicode and still work.
 */
...
void RangeType::normalize()
{
	if ((step_val > 0) && (end_val < begin_val)) {
		...
		printDeprecation("Using ranges of the form [begin:end] with begin value greater than the end "
										 "value is deprecated.");
	}
}
