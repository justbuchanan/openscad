...
bool Value::isDefined() const { return this->type() != ValueType::UNDEFINED; }
...
public:
	...
	std::string operator()(const boost::blank &) const { return "undef"; }
	...
	std::string operator()(const RangeType &v) const {
		return (boost::format("[%1% : %2% : %3%]") % v.begin_val % v.step_val %
		        v.end_val)
		    .str();
	}

	std::string operator()(const ValuePtr &v) const { return v->toString(); }
};
...
public:
	...
	std::string operator()(const RangeType &v) const {
		...
		if (steps >= 10000) {
			PRINTB(
			    "WARNING: Bad range parameter in for statement: too many "
			    "elements (%lu).",
			    steps);
			return "";
		}
...
bool Value::operator!=(const Value &v) const { return !(*this == v); }
...
public:
	...
	Value operator()(const Value::VectorType &op1,
	                 const Value::VectorType &op2) const {
...
Value Value::multmatvec(const VectorType &matrixvec,
                        const VectorType &vectorvec) {
	// Matrix * Vector
	VectorType dstv;
	for (size_t i = 0; i < matrixvec.size(); i++) {
		...
		for (size_t j = 0; j < matrixvec[i]->toVector().size(); j++) {
			if (matrixvec[i]->toVector()[j]->type() != ValueType::NUMBER ||
			    vectorvec[j]->type() != ValueType::NUMBER) {
				return Value();
			}
			r_e += matrixvec[i]->toVector()[j]->toDouble() *
			       vectorvec[j]->toDouble();
		}
...
Value Value::multvecmat(const VectorType &vectorvec,
                        const VectorType &matrixvec) {
	...
	for (size_t i = 0; i < matrixvec[0]->toVector().size(); i++) {
		...
		for (size_t j = 0; j < vectorvec.size(); j++) {
			...
			r_e += vectorvec[j]->toDouble() *
			       matrixvec[j]->toVector()[i]->toDouble();
		}
		dstv.push_back(ValuePtr(r_e));
	}
	return {dstv};
...
Value Value::operator*(const Value &v) const {
	if (this->type() == ValueType::NUMBER && v.type() == ValueType::NUMBER) {
		...
		if (vec1[0]->type() == ValueType::NUMBER &&
		    vec2[0]->type() == ValueType::NUMBER &&
		    vec1.size() == vec2.size()) {
			// Vector dot product.
			auto r = 0.0;
			for (size_t i = 0; i < vec1.size(); i++) {
				if (vec1[i]->type() != ValueType::NUMBER ||
				    vec2[i]->type() != ValueType::NUMBER) {
					return Value::undefined;
				}
		...
		} else if (vec1[0]->type() == ValueType::NUMBER &&
		           vec2[0]->type() == ValueType::VECTOR &&
		           vec1.size() == vec2.size()) {
...
Value Value::operator%(const Value &v) const {
	if (this->type() == ValueType::NUMBER && v.type() == ValueType::NUMBER) {
		return {
		    fmod(boost::get<double>(this->value), boost::get<double>(v.value))};
	}
	return Value::undefined;
}
...
public:
	Value operator()(const std::string &str, const double &idx) const {
		...
		if (i < str.size()) {
			// Ensure character (not byte) index is inside the character/glyph
			// array
			if (i < static_cast<unsigned int>(
			            g_utf8_strlen(str.c_str(), str.size()))) {
				gchar utf8_of_cp[6] = "";  // A buffer for a single unicode
				                           // character to be copied into
	...
	Value operator()(const RangeType &range, const double &idx) const {
		...
		switch (i) {
			case 0:
				return {range.begin_val};
			case 1:
				return {range.step_val};
			case 2:
				return {range.end_val};
		}
		return Value::undefined;
	}
...
void RangeType::normalize() {
	if ((step_val > 0) && (end_val < begin_val)) {
		...
		printDeprecation(
		    "Using ranges of the form [begin:end] with begin value greater "
...
RangeType::iterator::iterator(RangeType &range, type_t type)
    : range(range), val(range.begin_val), type(type) {
	update_type();
}
...
void RangeType::iterator::update_type() {
	...
	if (std::isnan(range.begin_val) || std::isnan(range.end_val) ||
	    std::isnan(range.step_val))
		type = type_t::RANGE_TYPE_END;
}

RangeType::iterator::reference RangeType::iterator::operator*() { return val; }
...
ValuePtr::ValuePtr() { this->reset(new Value()); }

ValuePtr::ValuePtr(const Value &v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(bool v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(int v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(double v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(const std::string &v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(const char *v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(const char v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(const Value::VectorType &v) { this->reset(new Value(v)); }

ValuePtr::ValuePtr(const RangeType &v) { this->reset(new Value(v)); }
...
ValuePtr ValuePtr::operator-() const { return ValuePtr(-**this); }
...
ValuePtr::operator bool() const { return **this; }

const Value &ValuePtr::operator*() const { return *this->get(); }
