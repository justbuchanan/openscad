...
/*! /class PolySet
        ...
        The PolySet class fulfils multiple tasks, partially for historical
   reasons.
...
PolySet::PolySet(unsigned int dim, boost::tribool convex)
    : dim(dim), convex(convex), dirty(false) {}

PolySet::PolySet(const Polygon2d &origin)
    : polygon(origin), dim(2), convex(unknown), dirty(false) {}
...
void PolySet::append_poly() { polygons.push_back(Polygon()); }
...
void PolySet::append(const PolySet &ps) {
    this->polygons.insert(this->polygons.end(), ps.polygons.begin(),
                          ps.polygons.end());
...
void PolySet::quantizeVertices() {
    ...
    for (std::vector<Polygon>::iterator iter = this->polygons.begin();
         iter != this->polygons.end();) {
        Polygon &p = *iter;
