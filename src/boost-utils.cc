...
// If the given (absolute) path is relative to the relative_to path, return a
// new relative path. Will normalize the given path first
fs::path boostfs_relative_path(const fs::path &path,
    ...
    while (*itr_path == *itr_relative_to && itr_path != p.end() &&
           itr_relative_to != r.end()) {
        ++itr_path;
        ++itr_relative_to;
    }
 ...
 *  The use case for this is any time you get a full path (from an open dialog,
 * perhaps) and want to store a relative path so that the group of files can be
 * moved to a different directory without breaking the paths. An IDE would be a
 * simple example, so that the project file could be safely checked out of
 * subversion."
 *
 * ALGORITHM:
...
fs::path boostfs_uncomplete(fs::path const p, fs::path const base) {
    ...
    /*!! this breaks stuff if path is a filename rather than a directory,
            which it most likely is... but then base shouldn't be a filename
       so... */
    ...
    if ((path_it == path_end) || (base_it == base_end)) {
        throw std::runtime_error(
            "path or base was empty; couldn't generate relative path");
    }
    ...
    while (true) {
        // compare all elements so far of path and base to find greatest common
        // root; when elements of path and base differ, or run out:
        if ((path_it == path_end) || (base_it == base_end) ||
            (*path_it != *base_it)) {
            // write to output, ../ times the number of remaining elements in
            // base; this is how far we've had to come down the tree from base
            // to get to the common root
