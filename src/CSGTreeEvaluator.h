...
public:
    CSGTreeEvaluator(const class Tree &tree,
                     class GeometryEvaluator *geomevaluator = nullptr)
    ...
    virtual Response visit(State &state,
                           const class AbstractIntersectionNode &node);
    ...
    const shared_ptr<CSGNode> &getRootNode() const { return this->rootNode; }
...
private:
    ...
    void applyToChildren(State &state, const AbstractNode &node,
                         OpenSCADOperator op);
    shared_ptr<CSGNode> evaluateCSGNodeFromGeometry(
        State &state, const shared_ptr<const class Geometry> &geom,
        const class ModuleInstantiation *modinst, const AbstractNode &node);
...
protected:
    ...
    std::map<int, shared_ptr<CSGNode>>
