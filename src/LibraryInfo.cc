...
std::string LibraryInfo::info() {
    ...
    std::string compiler_info("Clang " +
                              std::string(TOSTRING(__clang_version__)) + bits);
#else
...
#define OPENCSG_VERSION_STRING "unknown, <1.3.2"
    ...
    int status;
    cgal_3d_kernel =
        std::string(abi::__cxa_demangle(cgal_3d_kernel.c_str(), 0, 0, &status));
    cgal_2d_kernel =
        std::string(abi::__cxa_demangle(cgal_2d_kernel.c_str(), 0, 0, &status));
    cgal_2d_kernelEx = std::string(
        abi::__cxa_demangle(cgal_2d_kernelEx.c_str(), 0, 0, &status));
#endif  // demangle
    ...
    s << "OpenSCAD Version: " << openscad_detailedversionnumber
      ...
      << cgal_3d_kernel << ", " << cgal_2d_kernel << ", " << cgal_2d_kernelEx
    ...
    for (std::vector<std::string>::iterator it = librarypath.begin();
         it != librarypath.end(); it++) {
        s << "  " << *it << "\n";
    }
    ...
    for (std::vector<std::string>::iterator it = fontpath.begin();
         it != fontpath.end(); it++) {
        s << "  " << *it << "\n";
    }
