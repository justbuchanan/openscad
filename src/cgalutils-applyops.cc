...
namespace CGALUtils {
	...
	bool is_weakly_convex(Polyhedron const &p)
	{
		for (typename Polyhedron::Edge_const_iterator i = p.edges_begin(); i != p.edges_end(); ++i) {
			typename Polyhedron::Plane_3 p(i->opposite()->vertex()->point(), i->vertex()->point(),
																		 i->next()->vertex()->point());
		...
		std::unordered_set<typename Polyhedron::Facet_const_handle, typename CGAL::Handle_hash_function>
				visited;
		// c++11
	...
	*/
	CGAL_Nef_polyhedron *applyOperator(const Geometry::Geometries &children, OpenSCADOperator op)
	{
		...
		// and testdata/scad/bugs/issue204.scad
		catch (const CGAL::Failure_exception &e) {
			std::string opstr = op == OpenSCADOperator::INTERSECTION
															? "intersection"
	...
	bool applyHull(const Geometry::Geometries &children, PolySet &result)
	{
		...
		for (const auto &item : children) {
			...
			if (N) {
				if (!N->isEmpty()) {
					for (CGAL_Nef_polyhedron3::Vertex_const_iterator i = N->p3->vertices_begin();
			...
			while (++it != children.end()) {
				...
				for (size_t i = 0; i < 2; i++) {
					...
					if (ps)
						...
						nefworkaround::convert_to_Polyhedron<CGAL_Kernel3>(*nef->p3, poly);
					else
						throw 0;
					...
					if ((ps && ps->is_convex()) || (!ps && is_weakly_convex(poly))) {
						...
						if (ps) {
							PRINTDB("Minkowski: child %d is nonconvex PolySet, transforming to Nef and "
											"decomposing...",
											i);
					...
					for (size_t j = 0; j < P[1].size(); j++) {
						...
						for (int k = 0; k < 2; k++) {
							...
							for (CGAL_Polyhedron::Vertex_const_iterator pi = poly.vertices_begin();
								...
								points[k].push_back(
										Hull_kernel::Point_3(to_double(p[0]), to_double(p[1]), to_double(p[2])));
							}
						}
						...
						PRINTDB("Minkowski: Point cloud creation (%d ⨉ %d -> %d) took %f ms",
										points[0].size() % points[1].size() % minkowski_points.size() %
												(t.time() * 1000));
						t.reset();
				...
				if (result_parts.size() == 1) {
					...
					for (std::list<CGAL::Polyhedron_3<Hull_kernel>>::iterator i = result_parts.begin();
							 i != result_parts.end(); ++i) {
						PolySet ps(3, true);
						createPolySetFromPolyhedron(*i, ps);
						fake_children.push_back(
								std::make_pair((const AbstractNode *)nullptr,
