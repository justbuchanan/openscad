...
#if CGAL_VERSION_NR > CGAL_VERSION_NUMBER(4, 5, 1) || \
    CGAL_VERSION_NR < CGAL_VERSION_NUMBER(4, 5, 0)
...
bool is_weakly_convex(Polyhedron const &p) {
    for (typename Polyhedron::Edge_const_iterator i = p.edges_begin();
        ...
        if (p.has_on_positive_side(i->opposite()->next()->vertex()->point()) &&
            CGAL::squared_distance(
                p, i->opposite()->next()->vertex()->point()) > 1e-8) {
            return false;
        }
    }
    ...
    while (!to_explore.empty()) {
        ...
        to_explore.pop();
        typename Polyhedron::Facet::Halfedge_around_facet_const_circulator he,
            end;
...
/*!
        ...
        The child list should be guaranteed to contain non-NULL 3D or empty
   Geometry objects
*/
CGAL_Nef_polyhedron *applyOperator(const Geometry::Geometries &children,
                                   OpenSCADOperator op) {
    ...
    try {
        ...
        for (const auto &item : children) {
            ...
            if (!chN) {
                const PolySet *chps =
                    dynamic_cast<const PolySet *>(chgeom.get());
            ...
            switch (op) {
                ...
                case OpenSCADOperator::MINKOWSKI:
                    ...
                    break;
                default:
                    PRINTB("ERROR: Unsupported CGAL operator: %d",
                           static_cast<int>(op));
            }
        ...
        if (op == OpenSCADOperator::UNION && nary_union_num_inserted > 0) {
            N = new CGAL_Nef_polyhedron(
                new CGAL_Nef_polyhedron3(nary_union.get_union()));
        }
    }
    // union && difference assert triggered by
    // testdata/scad/bugs/rotate-diff-nonmanifold-crash.scad and
    // testdata/scad/bugs/issue204.scad
        ...
        std::string opstr =
            op == OpenSCADOperator::INTERSECTION
                ...
                : op == OpenSCADOperator::DIFFERENCE
                      ? "difference"
                      : op == OpenSCADOperator::UNION ? "union" : "UNKNOWN";
        PRINTB("ERROR: CGAL error in CGALUtils::applyBinaryOperator %s: %s",
               opstr % e.what());
    }
...
bool applyHull(const Geometry::Geometries &children, PolySet &result) {
    ...
    for (const auto &item : children) {
        ...
        const CGAL_Nef_polyhedron *N =
            dynamic_cast<const CGAL_Nef_polyhedron *>(chgeom.get());
        if (N) {
            if (!N->isEmpty()) {
                for (CGAL_Nef_polyhedron3::Vertex_const_iterator i =
                         N->p3->vertices_begin();
                     i != N->p3->vertices_end(); ++i) {
...
Geometry const *applyMinkowski(const Geometry::Geometries &children) {
    ...
    try {
        while (++it != children.end()) {
            ...
            for (size_t i = 0; i < 2; i++) {
                ...
                const CGAL_Nef_polyhedron *nef =
                    dynamic_cast<const CGAL_Nef_polyhedron *>(operands[i]);

                if (ps)
                    CGALUtils::createPolyhedronFromPolySet(*ps, poly);
                else if (nef && nef->p3->is_simple())
                    nefworkaround::convert_to_Polyhedron<CGAL_Kernel3>(*nef->p3,
                                                                       poly);
                else
                    throw 0;
                ...
                if ((ps && ps->is_convex()) ||
                    ...
                    if (ps) {
                        PRINTDB(
                            "Minkowski: child %d is nonconvex PolySet, "
                            "transforming to Nef and decomposing...",
                            i);
                        CGAL_Nef_polyhedron *p =
                            createNefPolyhedronFromGeometry(*ps);
                        if (!p->isEmpty()) decomposed_nef = *p->p3;
                        delete p;
                    } else {
                        PRINTDB(
                            "Minkowski: child %d is nonconvex Nef, "
                            "decomposing...",
                            i);
                    ...
                    // the first volume is the outer volume, which ignored in
                    // the decomposition
                ...
                for (size_t j = 0; j < P[1].size(); j++) {
                    ...
                    for (int k = 0; k < 2; k++) {
                        ...
                        for (CGAL_Polyhedron::Vertex_const_iterator pi =
                            ...
                            points[k].push_back(Hull_kernel::Point_3(
                                to_double(p[0]), to_double(p[1]),
                                to_double(p[2])));
                        }
                    }

                    minkowski_points.clear();
                    minkowski_points.reserve(points[0].size() *
                                             points[1].size());
                    for (size_t i = 0; i < points[0].size(); i++) {
                        for (size_t j = 0; j < points[1].size(); j++) {
                            minkowski_points.push_back(
                                points[0][i] + (points[1][j] - CGAL::ORIGIN));
                        }
                    }
                    ...
                    t.stop();
                    PRINTDB(
                        "Minkowski: Point cloud creation (%d ⨉ %d -> %d) took "
                        "%f ms",
                    ...
                    CGAL::convex_hull_3(minkowski_points.begin(),
                                        minkowski_points.end(), result);
                    ...
                    for (CGAL::Polyhedron_3<Hull_kernel>::Vertex_iterator i =
                        ...
                        CGAL::Polyhedron_3<Hull_kernel>::Vertex::Halfedge_handle
                            h,
                            e;
                        h = i->halfedge();
                        e = h;
                        ...
                        do {
                            ...
                            for (CGAL::Polyhedron_3<
                                     Hull_kernel>::Vertex::Halfedge_handle j =
                                     h->next_on_vertex();
                    ...
                    result.clear();
                    CGAL::convex_hull_3(strict_points.begin(),
                                        strict_points.end(), result);

                    t.stop();
                    PRINTDB("Minkowski: Computing convex hull took %f s",
                            t.time());
                    t.reset();
            ...
            if (result_parts.size() == 1) {
                ...
                t.start();
                PRINTDB("Minkowski: Computing union of %d parts",
                        result_parts.size());
                Geometry::Geometries fake_children;
                for (std::list<CGAL::Polyhedron_3<Hull_kernel>>::iterator i =
                         result_parts.begin();
                     i != result_parts.end(); ++i) {
                    PolySet ps(3, true);
                    createPolySetFromPolyhedron(*i, ps);
                    fake_children.push_back(std::make_pair(
                        (const AbstractNode *)nullptr,
                        shared_ptr<const Geometry>(
                            createNefPolyhedronFromGeometry(ps))));
                }
                CGAL_Nef_polyhedron *N = CGALUtils::applyOperator(
                    fake_children, OpenSCADOperator::UNION);
        ...
        CGAL_Nef_polyhedron *N =
            applyOperator(children, OpenSCADOperator::MINKOWSKI);
        CGAL::set_error_behaviour(old_behaviour);
        return N;
    }
}
