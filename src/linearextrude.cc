...
AbstractNode *LinearExtrudeModule::instantiate(const Context *ctx, const ModuleInstantiation *inst,
																							 EvalContext *evalctx) const
{
	auto node = new LinearExtrudeNode(inst);

	AssignmentList args{Assignment("file"),   Assignment("layer"), Assignment("height"),
											Assignment("origin"), Assignment("scale"), Assignment("center"),
	...
	if (!file->isUndefined() && file->type() == Value::ValueType::STRING) {
		printDeprecation("Support for reading files in linear_extrude will be removed in future "
	...
	if (node->twist != 0.0) {
		if (node->slices == 0) {
			node->slices = static_cast<int>(
					fmax(2, fabs(Calc::get_fragments_from_r(node->height, node->fn, node->fs, node->fa) *
											 node->twist / 360)));
		}
		node->has_twist = true;
	}
