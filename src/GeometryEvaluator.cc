	...
	if (!GeometryCache::instance()->contains(this->tree.getIdString(node))) {
		...
		if (!allownef) {
			if (shared_ptr<const CGAL_Nef_polyhedron> N =
							dynamic_pointer_cast<const CGAL_Nef_polyhedron>(this->root)) {
...
shared_ptr<const Geometry> GeometryEvaluator::smartCacheGet(const AbstractNode &node,
																														bool preferNef)
{
	const std::string &key = this->tree.getIdString(node);
	...
	if (state.isPostfix()) {
		...
		if (!isSmartCached(node)) {
			...
			if (geometry) {
				...
				const Polygon2d *result = ClipperUtils::applyOffset(*polygon, node.delta, node.join_type,
	...
	if (state.isPostfix()) {
		...
		if (!isSmartCached(node)) {
			if (matrix_contains_infinity(node.matrix) || matrix_contains_nan(node.matrix)) {
				...
				PRINT("WARNING: Transformation matrix contains Not-a-Number and/or Infinity - removing "
							"object.");
			} else {
				...
				if ((geom = res.constptr())) {
					if (geom->getDimension() == 2) {
						...
						if (res.isConst())
							...
							newpoly = dynamic_pointer_cast<Polygon2d>(res.ptr());

						Transform2d mat2;
						mat2.matrix() << node.matrix(0, 0), node.matrix(0, 1), node.matrix(0, 3),
								node.matrix(1, 0), node.matrix(1, 1), node.matrix(1, 3), node.matrix(3, 0),
								node.matrix(3, 1), node.matrix(3, 3);
...
static void add_slice(PolySet *ps, const Polygon2d &poly, double rot1, double rot2, double h1,
...
static Geometry *extrudePolygon(const LinearExtrudeNode &node, const Polygon2d &poly)
{
	...
	PolySet *ps = new PolySet(3, !cvx ? boost::tribool(false)
	...
	for (unsigned int j = 0; j < slices; j++) {
		...
		Vector2d scale1(1 - (1 - node.scale_x) * j / slices, 1 - (1 - node.scale_y) * j / slices);
	...
	if (state.isPostfix()) {
		...
		if (!isSmartCached(node)) {
			...
			if (!node.filename.empty()) {
				DxfData dxf(node.fn, node.fs, node.fa, node.filename, node.layername, node.origin_x,
										node.origin_y, node.scale_x);
...
static Geometry *rotatePolygon(const RotateExtrudeNode &node, const Polygon2d &poly)
{
	...
	for (const auto &o : poly.outlines()) {
		for (const auto &v : o.vertices) {
			...
			if ((max_x - min_x) > max_x && (max_x - min_x) > fabs(min_x)) {
				PRINTB("ERROR: all points for rotate_extrude() must have the same X coordinate sign (range "
							 "is %.2f -> %.2f)",
							 min_x % max_x);
				delete ps;
				return nullptr;
		...
		fragments = fmax(Calc::get_fragments_from_r(max_x - min_x, node.fn, node.fs, node.fa) *
	...
	if (node.angle != 360) {
		...
		Transform3d rot2(Eigen::AngleAxisd(node.angle * M_PI / 180, Vector3d::UnitZ()) *
...
// It's better in V6 but not quite there. FIXME: stand-alone example.
				...
				if (newgeom) {
					shared_ptr<const CGAL_Nef_polyhedron> Nptr =
							dynamic_pointer_cast<const CGAL_Nef_polyhedron>(newgeom);
					if (!Nptr) {
			...
			case CgaladvType::RESIZE: {
				...
				if (geom) {
					...
					shared_ptr<CGAL_Nef_polyhedron> N =
							dynamic_pointer_cast<CGAL_Nef_polyhedron>(editablegeom);
					if (N) {
