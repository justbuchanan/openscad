...
GeometryEvaluator::GeometryEvaluator(const class Tree &tree) : tree(tree) {}
...
shared_ptr<const Geometry> GeometryEvaluator::evaluateGeometry(
    ...
    if (!GeometryCache::instance()->contains(this->tree.getIdString(node))) {
        ...
        if (!allownef) {
            if (shared_ptr<const CGAL_Nef_polyhedron> N =
                    dynamic_pointer_cast<const CGAL_Nef_polyhedron>(
                        this->root)) {
                ...
                if (!N->isEmpty()) {
                    bool err =
                        CGALUtils::createPolySetFromNefPolyhedron3(*N->p3, *ps);
                    if (err) {
...
GeometryEvaluator::ResultObject GeometryEvaluator::applyToChildren(
    const AbstractNode &node, OpenSCADOperator op) {
    unsigned int dim = 0;
...
/*!
        ...
        May return nullptr or any 3D Geometry object (can be either PolySet or
   CGAL_Nef_polyhedron)
*/
GeometryEvaluator::ResultObject GeometryEvaluator::applyToChildren3D(
    ...
    if (op == OpenSCADOperator::MINKOWSKI) {
        ...
        if (actualchildren.size() == 1)
            return ResultObject(actualchildren.front().second);
...
*/
std::vector<const class Polygon2d *> GeometryEvaluator::collectChildren2D(
    ...
    for (const auto &item : this->visitedchildren[node.index()]) {
        ...
        if (chgeom) {
            if (chgeom->getDimension() == 2) {
                const Polygon2d *polygons =
                    dynamic_cast<const Polygon2d *>(chgeom.get());
                assert(polygons);
...
/*!
        Since we can generate both Nef and non-Nef geometry, we need to insert
   it into the appropriate cache. This method inserts the geometry into the
   appropriate cache if it's not already cached.
*/
void GeometryEvaluator::smartCacheInsert(
    const AbstractNode &node, const shared_ptr<const Geometry> &geom) {
...
shared_ptr<const Geometry> GeometryEvaluator::smartCacheGet(
    const AbstractNode &node, bool preferNef) {
    const std::string &key = this->tree.getIdString(node);
...
*/
Polygon2d *GeometryEvaluator::applyToChildren2D(const AbstractNode &node,
                                                OpenSCADOperator op) {
...
/*!
        ...
        Usually, this should be called from the postfix stage, but for some
   nodes, we defer traversal letting other components (e.g. CGAL) render the
   subgraph, and we'll then call this from prefix and prune further traversal.
        ...
        if (!isSmartCached(node)) {
            ...
            if (geometry) {
                const Polygon2d *polygon =
                    dynamic_cast<const Polygon2d *>(geometry);
                // ClipperLib documentation: The formula for the number of steps
                // in a full circular arc is ... Pi / acos(1 - arc_tolerance /
                // abs(delta))
                double n = Calc::get_fragments_from_r(
                    std::abs(node.delta), node.fn, node.fs, node.fa);
                double arc_tolerance =
                    std::abs(node.delta) * (1 - cos(M_PI / n));
                const Polygon2d *result = ClipperUtils::applyOffset(
                    *polygon, node.delta, node.join_type, node.miter_limit,
                    arc_tolerance);
                assert(result);
                geom.reset(result);
                delete geometry;
...
Response GeometryEvaluator::visit(State &state, const RenderNode &node) {
    ...
    if (state.isPostfix()) {
        ...
        if (!isSmartCached(node)) {
            ...
            geom = res.constptr();
            if (shared_ptr<const PolySet> ps =
                    dynamic_pointer_cast<const PolySet>(geom)) {
                // If we got a const object, make a copy
                shared_ptr<PolySet> newps;
                if (res.isConst())
                    newps.reset(new PolySet(*ps));
                else
                    newps = dynamic_pointer_cast<PolySet>(res.ptr());
                newps->setConvexity(node.convexity);
                geom = newps;
            } else if (shared_ptr<const CGAL_Nef_polyhedron> N =
                           dynamic_pointer_cast<const CGAL_Nef_polyhedron>(
                               geom)) {
                // If we got a const object, make a copy
                shared_ptr<CGAL_Nef_polyhedron> newN;
                if (res.isConst())
                    newN.reset((CGAL_Nef_polyhedron *)N->copy());
                else
                    newN = dynamic_pointer_cast<CGAL_Nef_polyhedron>(res.ptr());
    ...
    if (state.isPostfix()) {
        ...
        if (!isSmartCached(node)) {
            if (matrix_contains_infinity(node.matrix) ||
                ...
                // due to the way parse/eval works we can't currently
                // distinguish between NaN and Inf
                PRINT(
                    "WARNING: Transformation matrix contains Not-a-Number "
                ...
                if ((geom = res.constptr())) {
                    if (geom->getDimension() == 2) {
                        shared_ptr<const Polygon2d> polygons =
                            dynamic_pointer_cast<const Polygon2d>(geom);
                        assert(polygons);
                        ...
                        if (res.isConst())
                            ...
                            newpoly =
                                dynamic_pointer_cast<Polygon2d>(res.ptr());

                        Transform2d mat2;
                        mat2.matrix() << node.matrix(0, 0), node.matrix(0, 1),
                            node.matrix(0, 3), node.matrix(1, 0),
                            node.matrix(1, 1), node.matrix(1, 3),
                            node.matrix(3, 0), node.matrix(3, 1),
                            node.matrix(3, 3);
                        newpoly->transform(mat2);
                        // A 2D transformation may flip the winding order of a
                        // polygon. If that happens with a sanitized polygon, we
                        // need to reverse the winding order for it to be
                        // correct.
                        if (newpoly->isSanitized() &&
                            mat2.matrix().determinant() <= 0) {
                        ...
                        if (ps) {
                            ...
                            if (res.isConst())
                                ...
                                newps =
                                    dynamic_pointer_cast<PolySet>(res.ptr());
                            ...
                            if (res.isConst())
                                ...
                                newN =
                                    dynamic_pointer_cast<CGAL_Nef_polyhedron>(
                                        res.ptr());
...
static void add_slice(PolySet *ps, const Polygon2d &poly, double rot1,
                      double rot2, double h1, double h2, const Vector2d &scale1,
                      const Vector2d &scale2) {
    Eigen::Affine2d trans1(Eigen::Scaling(scale1) *
                           Eigen::Rotation2D<double>(-rot1 * M_PI / 180));
    Eigen::Affine2d trans2(Eigen::Scaling(scale2) *
                           Eigen::Rotation2D<double>(-rot2 * M_PI / 180));
...
/*!
        Input to extrude should be sanitized. This means non-intersecting,
   correct winding order etc., the input coming from a library like Clipper.
*/
static Geometry *extrudePolygon(const LinearExtrudeNode &node,
                                const Polygon2d &poly) {
    bool cvx = poly.is_convex();
    PolySet *ps =
        new PolySet(3, !cvx ? boost::tribool(false)
                            : node.twist == 0 ? boost::tribool(true) : unknown);
    ...
    if (node.scale_x > 0 || node.scale_y > 0) {
        ...
        Eigen::Affine2d trans(
            Eigen::Scaling(node.scale_x, node.scale_y) *
    ...
    if (state.isPostfix()) {
        ...
        if (!isSmartCached(node)) {
            ...
            if (!node.filename.empty()) {
                DxfData dxf(node.fn, node.fs, node.fa, node.filename,
                            node.layername, node.origin_x, node.origin_y,
                            node.scale_x);
...
/*!
  ...
  o 2D polygon having an edge being on the Y axis:
          In this case, we don't need to generate geometry involving this edge
  as it will be an internal edge. o 2D polygon having a vertex touching the Y
  axis: This is more complex as the resulting geometry will (may?) be
  nonmanifold. In any case, the previous case is a specialization of this, so
  the following should be handled for both cases: Since the ring associated with
  this vertex will have a radius of zero, it will collapse to one vertex. Any
  quad using this ring will be collapsed to a triangle.

        Currently, we generate a lot of zero-area triangles

*/
static Geometry *rotatePolygon(const RotateExtrudeNode &node,
                               const Polygon2d &poly) {
    ...
    for (const auto &o : poly.outlines()) {
        for (const auto &v : o.vertices) {
            ...
            if ((max_x - min_x) > max_x && (max_x - min_x) > fabs(min_x)) {
                PRINTB(
                    "ERROR: all points for rotate_extrude() must have the same "
                    "X coordinate sign (range is %.2f -> %.2f)",
                    min_x % max_x);
                delete ps;
                return nullptr;
            }
        }
        fragments = fmax(Calc::get_fragments_from_r(max_x - min_x, node.fn,
                                                    node.fs, node.fa) *
                             std::abs(node.angle) / 360,
                         1);
    }

    bool flip_faces = (min_x >= 0 && node.angle > 0 && node.angle != 360) ||
                      (min_x < 0 && (node.angle < 0 || node.angle == 360));
    ...
    if (node.angle != 360) {
        ...
        Transform3d rot2(
            Eigen::AngleAxisd(node.angle * M_PI / 180, Vector3d::UnitZ()) *
            Eigen::AngleAxisd(M_PI / 2, Vector3d::UnitX()));
    ...
    for (const auto &o : poly.outlines()) {
        ...
        fill_ring(rings[0], o, (node.angle == 360) ? -M_PI / 2 : M_PI / 2,
                  flip_faces);  // first ring
        for (int j = 0; j < fragments; j++) {
            double a;
            if (node.angle == 360)
                a = -M_PI / 2 +
                    ((j + 1) % fragments * 2 * M_PI) /
                        fragments;  // start on the -X axis, for legacy support
            else
                a = M_PI / 2 - (j + 1) * (node.angle * M_PI / 180) /
                                   fragments;  // start on the X axis
...
// It's better in V6 but not quite there. FIXME: stand-alone example.
                    ...
                    if (!chPS) {
                        ...
                        if (chN) {
                            PolySet *ps = new PolySet(3);
                            bool err =
                                CGALUtils::createPolySetFromNefPolyhedron3(
                                    *chN->p3, *ps);
                            if (err) {
                                PRINT("ERROR: Nef->PolySet failed");
                            } else {
                    ...
                    if (poly) {
                        ...
                        // Using NonZero ensures that we don't create holes from
                        // polygons sharing edges since we're unioning a mesh
                        result =
                            ClipperUtils::process(result, ClipperLib::ctUnion,
                                                  ClipperLib::pftNonZero);
                        // Add correctly winded polygons to the main clipper
                        sumclipper.AddPaths(result, ClipperLib::ptSubject,
                                            true);
                    }
                ...
                // This is key - without StrictlySimple, we tend to get
                // self-intersecting results
                sumclipper.StrictlySimple(true);
                sumclipper.Execute(ClipperLib::ctUnion, sumresult,
                                   ClipperLib::pftNonZero,
                                   ClipperLib::pftNonZero);
                if (sumresult.Total() > 0)
                    geom.reset(ClipperUtils::toPolygon2d(sumresult));
            } else {
                shared_ptr<const Geometry> newgeom =
                    applyToChildren3D(node, OpenSCADOperator::UNION).constptr();
                if (newgeom) {
                    shared_ptr<const CGAL_Nef_polyhedron> Nptr =
                        dynamic_pointer_cast<const CGAL_Nef_polyhedron>(
                            newgeom);
                    if (!Nptr) {
                        Nptr.reset(CGALUtils::createNefPolyhedronFromGeometry(
                            *newgeom));
                    }
                    if (!Nptr->isEmpty()) {
                        Polygon2d *poly =
                            CGALUtils::project(*Nptr, node.cut_mode);
                        if (poly) {
        ...
        if (!isSmartCached(node)) {
            switch (node.type) {
                case CgaladvType::MINKOWSKI: {
                    ResultObject res =
                        applyToChildren(node, OpenSCADOperator::MINKOWSKI);
                    geom = res.constptr();
                    ...
                    if (geom && geom->getConvexity() != node.convexity) {
                        ...
                        // If we got a const object, make a copy
                        if (res.isConst())
                            editablegeom.reset(geom->copy());
                        else
                            editablegeom = res.ptr();
                        geom = editablegeom;
                ...
                case CgaladvType::HULL: {
                    geom = applyToChildren(node, OpenSCADOperator::HULL)
                               .constptr();
                    break;
                }
                case CgaladvType::RESIZE: {
                    ...
                    if (geom) {
                        ...
                        if (N) {
                            ...
                            shared_ptr<Polygon2d> poly =
                                dynamic_pointer_cast<Polygon2d>(editablegeom);
                            if (poly) {
                                poly->resize(
                                    Vector2d(node.newsize[0], node.newsize[1]),
                                    Eigen::Matrix<bool, 2, 1>(
                                        node.autosize[0], node.autosize[1]));
                            } else {
                                shared_ptr<PolySet> ps =
                                    dynamic_pointer_cast<PolySet>(editablegeom);
                                if (ps) {
...
Response GeometryEvaluator::visit(State &state,
    ...
    if (state.isPostfix()) {
        shared_ptr<const class Geometry> geom;
        if (!isSmartCached(node)) {
            geom = applyToChildren(node, OpenSCADOperator::INTERSECTION)
                       .constptr();
        } else {
            geom = smartCacheGet(node, state.preferNef());
        }
