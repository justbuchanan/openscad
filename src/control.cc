...
public:  // methods
    ...
    ControlModule(Type type, const Feature &feature)
        : AbstractModule(feature), type(type) {}

    virtual AbstractNode *instantiate(const Context *ctx,
                                      const ModuleInstantiation *inst,
                                      EvalContext *evalctx) const;

    static void for_eval(AbstractNode &node, const ModuleInstantiation &inst,
                         size_t l, const Context *ctx,
                         const EvalContext *evalctx);

    static const EvalContext *getLastModuleCtx(const EvalContext *evalctx);

    static AbstractNode *getChild(const ValuePtr &value,
                                  const EvalContext *modulectx);
...
void ControlModule::for_eval(AbstractNode &node,
                             const ModuleInstantiation &inst, size_t l,
    ...
    if (evalctx->numArgs() > l) {
        ...
        if (it_values->type() == Value::ValueType::RANGE) {
            ...
            if (steps >= 10000) {
                ...
                for (RangeType::iterator it = range.begin(); it != range.end();
                     it++) {
        ...
        // At this point, the for loop variables have been set and we can
        // initialize the local scope (as they may depend on the for loop
        // variables
        Context c(ctx);
...
const EvalContext *ControlModule::getLastModuleCtx(const EvalContext *evalctx) {
    ...
    while (tmpc->getParent()) {
        const ModuleContext *modulectx =
            dynamic_cast<const ModuleContext *>(tmpc->getParent());
        if (modulectx) {
            // This will trigger if trying to invoke child from the root of any
            // file assert(filectx->evalctx);
...
// static
AbstractNode *ControlModule::getChild(const ValuePtr &value,
                                      const EvalContext *modulectx) {
    if (value->type() != Value::ValueType::NUMBER) {
        // Invalid parameter
        // (e.g. first child of difference is invalid)
        PRINTB(
            "WARNING: Bad parameter type (%s) for children, only accept: "
            "empty, number, vector, range.",
            value->toString());
        return nullptr;
    }
    double v;
...
AbstractNode *ControlModule::instantiate(const Context * /*ctx*/,
                                         const ModuleInstantiation *inst,
                                         EvalContext *evalctx) const {
    ...
    switch (this->type) {
        case Type::CHILD: {
            printDeprecation(
                "child() will be removed in future releases. Use children() "
                "instead.");
            int n = 0;
            if (evalctx->numArgs() > 0) {
                ...
                if (evalctx->getArgValue(0)->getDouble(v)) {
                    n = trunc(v);
                    if (n < 0) {
                        PRINTB("WARNING: Negative child index (%d) not allowed",
                               n);
                        return nullptr;  // Disallow negative child indices
                    }
                }
            ...
            if (evalctx->numArgs() <= 0) {
                ...
                for (int n = 0; n < (int)modulectx->numChildren(); ++n) {
                    AbstractNode *childnode =
                        modulectx->getChild(n)->evaluate(modulectx);
                ...
                if (value->type() == Value::ValueType::NUMBER) {
                    ...
                    if (steps >= 10000) {
                        PRINTB(
                            "WARNING: Bad range parameter for children: too "
                            "many elements (%lu).",
                            steps);
                        return nullptr;
                    }
                    AbstractNode *node = new GroupNode(inst);
                    for (RangeType::iterator it = range.begin();
                         it != range.end(); it++) {
                        AbstractNode *childnode = getChild(
                            ValuePtr(*it), modulectx);       // with error cases
                        if (childnode == nullptr) continue;  // error
                        node->children.push_back(childnode);
                    }
                    return node;
                    ...
                    // (e.g. first child of difference is invalid)
                    PRINTB(
                        "WARNING: Bad parameter type (%s) for children, only "
                        "accept: empty, number, vector, range.",
                        value->toString());
                    return nullptr;
                }
            }
        ...
        case Type::LET: {
            ...
            node->children.insert(node->children.end(),
                                  instantiatednodes.begin(),
                                  instantiatednodes.end());
        } break;

        case Type::ASSIGN: {
            node = new GroupNode(inst);
            // We create a new context to avoid parameters from influencing each
            // other
            // -> parallel evaluation. This is to be backwards compatible.
            Context c(evalctx);
            for (size_t i = 0; i < evalctx->numArgs(); i++) {
                if (!evalctx->getArgName(i).empty())
                    c.set_variable(evalctx->getArgName(i),
        ...
        case Type::IF: {
            node = new GroupNode(inst);
            const IfElseModuleInstantiation *ifelse =
                dynamic_cast<const IfElseModuleInstantiation *>(inst);
            if (evalctx->numArgs() > 0 && evalctx->getArgValue(0)->toBool()) {
                inst->scope.apply(*evalctx);
                std::vector<AbstractNode *> instantiatednodes =
                    ifelse->instantiateChildren(evalctx);
                node->children.insert(node->children.end(),
                                      instantiatednodes.begin(),
                                      instantiatednodes.end());
            } else {
                ...
                std::vector<AbstractNode *> instantiatednodes =
                    ifelse->instantiateElseChildren(evalctx);
                node->children.insert(node->children.end(),
                                      instantiatednodes.begin(),
                                      instantiatednodes.end());
            }
...
void register_builtin_control() {
    ...
    Builtins::init("children",
                   new ControlModule(ControlModule::Type::CHILDREN));
    Builtins::init("echo", new ControlModule(ControlModule::Type::ECHO));
    Builtins::init("assert",
                   new ControlModule(ControlModule::Type::ASSERT,
                                     Feature::ExperimentalAssertExpression));
    ...
    Builtins::init("intersection_for",
                   new ControlModule(ControlModule::Type::INT_FOR));
