...
{
public: // types
	enum class Type { CHILD, CHILDREN, ECHO, ASSERT, ASSIGN, FOR, LET, INT_FOR, IF };
...
AbstractNode *ControlModule::getChild(const ValuePtr &value, const EvalContext *modulectx)
{
	if (value->type() != Value::ValueType::NUMBER) {
		...
		PRINTB(
				"WARNING: Bad parameter type (%s) for children, only accept: empty, number, vector, range.",
				value->toString());
		return nullptr;
	}
	double v;
...
AbstractNode *ControlModule::instantiate(const Context * /*ctx*/, const ModuleInstantiation *inst,
																				 EvalContext *evalctx) const
{
	...
	case Type::CHILDREN: {
		...
		if (evalctx->numArgs() <= 0) {
			...
			if (value->type() == Value::ValueType::NUMBER) {
				...
				// Invalid parameter
				// (e.g. first child of difference is invalid)
				PRINTB("WARNING: Bad parameter type (%s) for children, only accept: empty, number, vector, "
							 "range.",
							 value->toString());
				return nullptr;
			}
