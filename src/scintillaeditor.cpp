...
void ScintillaEditor::applySettings()
{
	...
	qsci->setWrapIndentMode(
			conv.toLineWrapIndentationStyle(s->get(Settings::Settings::lineWrapIndentationStyle)));
	qsci->setWrapVisualFlags(
			conv.toLineWrapVisualization(s->get(Settings::Settings::lineWrapVisualizationEnd)),
	...
	qsci->setBraceMatching(s->get(Settings::Settings::enableBraceMatching).toBool()
...
void ScintillaEditor::noColor()
{
	...
	qsci->setIndicatorOutlineColor(QColor(0, 0, 0, 255),
																 errorIndicatorNumber); // only alpha part is used
	qsci->setIndicatorForegroundColor(QColor(255, 255, 0, 128), findIndicatorNumber); // yellow
	qsci->setIndicatorOutlineColor(QColor(0, 0, 0, 255),
																 findIndicatorNumber); // only alpha part is used
		...
		for (fs::directory_iterator dir_iter(color_schemes); dir_iter != end_iter; ++dir_iter) {
			...
			if (colorScheme->valid()) {
				result_set.insert(colorscheme_set_t::value_type(
						colorScheme->index(), shared_ptr<EditorColorScheme>(colorScheme)));
			} else {
				delete colorScheme;
			}
		}
...
QStringList ScintillaEditor::colorSchemes()
{
	...
	for (colorscheme_set_t::const_iterator it = colorscheme_set.begin(); it != colorscheme_set.end();
			 it++) {
...
int ScintillaEditor::resetFindIndicators(const QString &findText, bool visibility)
{
	...
	qsci->clearIndicatorRange(clearlineFrom, clearindexFrom, clearlineTo, clearindexTo,
														findIndicatorNumber);
	if (visibility && qsci->findFirst(findText, false /*re*/, false /*cs*/, false /*wo*/,
																		false /*wrap*/, true /*forward*/, 0, 0, false)) {
		findwordcount++;
...
bool ScintillaEditor::find(const QString &expr, bool findNext, bool findBackwards)
{
	...
	if (qsci->hasSelectedText()) {
		...
		startline =
				!(findBackwards xor findNext) ? std::min(lineFrom, lineTo) : std::max(lineFrom, lineTo);
		startindex =
...
bool ScintillaEditor::modifyNumber(int key)
{
	...
	long long int number =
			(dotpos < 0) ? nr.toLongLong() : (nr.left(dotpos) + nr.mid(dotpos + 1)).toLongLong();
