...
/*!
        ...
        A visitor responsible for creating a binary tree of CSGNode nodes used
   for rendering with OpenCSG.
*/
...
shared_ptr<CSGNode> CSGTreeEvaluator::evaluateCSGNodeFromGeometry(
    ...
    // We cannot render Polygon2d directly, so we preprocess (tessellate) it
    // here
    auto g = geom;
    if (!g->isEmpty()) {
        ...
        if (p2d) {
            ...
            // tessellate also in the indeterminate state so we cannot just use
            // a boolean comparison. See #1061
    ...
    shared_ptr<CSGNode> t(
        new CSGLeaf(g, state.matrix(), state.color(), stream.str()));
...
Response CSGTreeEvaluator::visit(State &state, const AbstractPolyNode &node) {
    if (state.isPostfix()) {
        ...
        if (this->geomevaluator) {
            ...
            if (geom) {
                t1 = evaluateCSGNodeFromGeometry(state, geom, node.modinst,
                                                 node);
            }
...
Response CSGTreeEvaluator::visit(State &state, const TransformNode &node) {
    if (state.isPrefix()) {
        if (matrix_contains_infinity(node.matrix) ||
            ...
            PRINT(
                "WARNING: Transformation matrix contains Not-a-Number and/or "
                "Infinity - removing object.");
...
Response CSGTreeEvaluator::visit(State &state, const CgaladvNode &node) {
    if (state.isPostfix()) {
        shared_ptr<CSGNode> t1;
        // FIXME: Calling evaluator directly since we're not a PolyNode.
        // Generalize this.
        shared_ptr<const Geometry> geom;
        if (this->geomevaluator) {
            geom = this->geomevaluator->evaluateGeometry(node, false);
