...
std::string QGLView::getRendererInfo() const {
    ...
    info << glew_dump();
    // Don't translate as translated text in the Library Info dialog is not
    // wanted
#ifdef USE_QOPENGLWIDGET
...
void QGLView::display_opencsg_warning_dialog() {
    ...
    if (this->is_opencsg_capable) {
        message +=
            _("Warning: You may experience OpenCSG rendering errors.\n\n");
    } else {
        message +=
            _("Warning: Missing OpenGL capabilities for OpenCSG - OpenCSG has "
              "been disabled.\n\n");
    ...
    dialog->enableOpenCSGBox->setChecked(
        Preferences::inst()
...
void QGLView::resizeGL(int w, int h) { GLView::resizeGL(w, h); }
    ...
    if (mouse_drag_active) {
        if (event->buttons() & Qt::LeftButton
            ...
            if ((QApplication::keyboardModifiers() & Qt::ShiftModifier) != 0) {
                ...
                aax = Eigen::AngleAxisd(-(cam.object_rot.x() / 180) * M_PI,
                                        Vector3d::UnitX());
                aay = Eigen::AngleAxisd(-(cam.object_rot.y() / 180) * M_PI,
                                        Vector3d::UnitY());
                aaz = Eigen::AngleAxisd(-(cam.object_rot.z() / 180) * M_PI,
                                        Vector3d::UnitZ());
                ...
                for (int i = 0; i < 3; i++)
                    for (int j = 0; j < 3; j++) tm(j, i) = tm3(j, i);

                Matrix4d vec;
                vec << 0, 0, 0, mx, 0, 0, 0, my, 0, 0, 0, mz, 0, 0, 0, 1;
                tm = tm * vec;
