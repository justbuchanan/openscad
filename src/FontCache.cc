...
FontInfo::FontInfo(const std::string &family, const std::string &style,
...
FontCache::FontCache() {
	...
	// config by overriding the built-in fontconfig path. For system installs
	// and dev environments, we leave this alone
	fs::path fontdir(PlatformUtils::resourcePath("fonts"));
	if (fs::is_regular_file(fontdir / "fonts.conf")) {
		PlatformUtils::setenv("FONTCONFIG_PATH",
		                      (fs::absolute(fontdir).generic_string()).c_str(),
		                      0);
	}
	...
	if (!this->config) {
		PRINT(
		    "WARNING: Can't initialize fontconfig library, text() objects will "
		    "not be rendered");
		return;
	}
	...
	if (error) {
		PRINT(
		    "WARNING: Can't initialize freetype library, text() objects will "
		    "not be rendered");
		return;
	}
...
FontInfoList *FontCache::list_fonts() const {
	FcObjectSet *object_set =
	    FcObjectSetBuild(FC_FAMILY, FC_STYLE, FC_FILE, (char *)0);
...
void FontCache::dump_cache(const std::string &info) {
	std::cout << info << ":";
	for (cache_t::iterator it = this->cache.begin(); it != this->cache.end();
	     it++) {
...
FT_Face FontCache::find_face_fontconfig(const std::string &font) const {
	...
	FT_Face face;
	FT_Error error = FT_New_Face(this->library, (const char *)file_value.u.s,
	                             font_index.u.i, &face);
	...
	if (FT_Select_Charmap(face, ft_encoding_unicode) == 0) {
		...
		if (!charmap_set)
			charmap_set =
			    try_charmap(face, TT_PLATFORM_MACINTOSH, TT_MAC_ID_ROMAN);
		if (!charmap_set)
			charmap_set = try_charmap(face, TT_PLATFORM_ISO, TT_ISO_ID_8859_1);
		if (!charmap_set)
			charmap_set =
			    try_charmap(face, TT_PLATFORM_ISO, TT_ISO_ID_7BIT_ASCII);
		if (!charmap_set)
...
bool FontCache::try_charmap(FT_Face face, int platform_id,
		...
		if ((charmap->platform_id == platform_id) &&
			...
			if (FT_Set_Charmap(face, charmap) == 0) {
				...
				if (is_windows_symbol_font(face)) {
					PRINTDB(
					    "Detected windows symbol font with character codes in "
					    "the Private Use Area of Unicode at 0xf000: %s/%s",
					    face->family_name % face->style_name);
				}
				return true;
			}
