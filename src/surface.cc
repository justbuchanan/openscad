...
typedef std::unordered_map<std::pair<int, int>, double,
...
AbstractNode *SurfaceModule::instantiate(const Context *ctx,
                                         const ModuleInstantiation *inst,
                                         EvalContext *evalctx) const {
    ...
    AssignmentList args{Assignment("file"), Assignment("center"),
    ...
    auto filename =
        lookup_file(fileval->isUndefined() ? "" : fileval->toString(),
                    inst->path(), c.documentPath());
...
void SurfaceNode::convert_image(img_data_t &data, std::vector<uint8_t> &img,
    ...
    for (unsigned int y = 0; y < height; y++) {
        for (unsigned int x = 0; x < width; x++) {
            ...
            double pixel = 0.2126 * img[idx] + 0.7152 * img[idx + 1] +
                           0.0722 * img[idx + 2];
...
img_data_t SurfaceNode::read_dat(std::string filename) const {
    ...
    while (!stream.eof()) {
        ...
        try {
            ...
            if (!stream.eof()) {
                PRINTB("WARNING: Illegal value in '%s': %s",
                       filename % blc.what());
            }
            break;
        }
...
const Geometry *SurfaceNode::createGeometry() const {
    ...
    for (int i = 1; i < lines; i++) {
        ...
        p->insert_vertex(ox + columns - 1, oy + i - 1,
                         data[std::make_pair(i - 1, columns - 1)]);
        p->insert_vertex(ox + columns - 1, oy + i,
                         data[std::make_pair(i, columns - 1)]);
    ...
    for (int i = 1; i < columns; i++) {
        ...
        p->append_vertex(ox + i - 1, oy + lines - 1,
                         data[std::make_pair(lines - 1, i - 1)]);
        p->append_vertex(ox + i, oy + lines - 1,
                         data[std::make_pair(lines - 1, i)]);
