...
//
// $URL:
// svn+ssh://scm.gforge.inria.fr/svn/cgal/branches/releases/CGAL-4.0-branch/Nef_3/include/CGAL/Nef_helper_3.h
...
class Triangulation_handler2 {
	...
	CGAL::Unique_hash_map<CTVertex_handle, typename Nef::Vertex_const_handle>
	    ctv2v;
...
public:
	Triangulation_handler2(typename Nef::Halffacet_const_handle f)
		...
		for (fci = f->facet_cycles_begin(); fci != f->facet_cycles_end();
		     ++fci) {
			if (fci.is_shalfedge()) {
				typename Nef::SHalfedge_around_facet_const_circulator sfc(fci),
				    send(sfc);
		...
		do {
		    ...
		    !ct.is_constrained(typename CT::Edge(vc, vc->index(opposite))));
	...
	bool get_next_triangle(Triangle_3& tr) {
		...
		tr = Triangle_3(fi->vertex(0)->point(), fi->vertex(1)->point(),
		                fi->vertex(2)->point());
		++fi;
		return true;
	}
...
public:
	...
	public:
		...
		void visit(typename Nef::Halffacet_const_handle opposite_facet) {
			...
			typename Nef::Halffacet_const_handle f =
			    !opposite_facet->incident_volume()->mark()
			        ? opposite_facet
			        : opposite_facet->twin();

			typename Nef::SHalfedge_around_facet_const_circulator sfc1(
			    f->facet_cycles_begin()),
			    sfc2(sfc1);
			...
			if (++f->facet_cycles_begin() != f->facet_cycles_end() ||
				...
				try {  // added for OpenSCAD
				    ...
				    const CGAL::Failure_exception& e) {  // added for OpenSCAD
					PRINTB(
					    "WARNING: CGAL NefPolyhedron Triangulation failed: %s",
					    e.what());       // added for OpenSCAD
				...
				CGAL_assertion(se != 0);
				typename Nef::SHalfedge_around_facet_const_circulator hc_start(
				    se);
				typename Nef::SHalfedge_around_facet_const_circulator hc_end(
				    hc_start);
				CGAL_For_all(hc_start, hc_end) {
					...
					B.add_vertex_to_facet(
					    VI[hc_start->source()->center_vertex()]);
				}
				B.end_facet();
			}
		}
...
public:
	...
	Build_polyhedron(const typename Nef::SNC_const_decorator& s)
	    : error(false), scd(s), VI(s.vertices_begin(), s.vertices_end(), 'V') {}
	...
	void operator()(HDS& hds) {
		...
		CGAL_forall_volumes(c, scd) if (skip_volumes-- <= 0) {
			scd.visit_shell_objects(
			    typename Nef::SFace_const_handle(c->shells_begin()), V);
		}
		B.end_surface();
...
bool convert_to_Polyhedron(const CGAL::Nef_polyhedron_3<Kernel>& N,
