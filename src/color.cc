...
AbstractNode *ColorModule::instantiate(const Context *ctx, const ModuleInstantiation *inst,
																			 EvalContext *evalctx) const
{
	...
	if (v->type() == Value::ValueType::VECTOR) {
		for (size_t i = 0; i < 4; i++) {
			...
			if (node->color[i] > 1)
				PRINTB_NOCACHE(
						"WARNING: color() expects numbers between 0.0 and 1.0. Value of %.1f is too large.",
						node->color[i]);
		}
...
std::string ColorNode::toString() const
{
	...
	stream << "color([" << this->color[0] << ", " << this->color[1] << ", " << this->color[2] << ", "
				 << this->color[3] << "])";
