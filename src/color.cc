...
AbstractNode *ColorModule::instantiate(const Context *ctx,
    ...
    if (v->type() == Value::ValueType::VECTOR) {
        for (size_t i = 0; i < 4; i++) {
            node->color[i] =
                i < v->toVector().size() ? v->toVector()[i]->toDouble() : 1.0;
            if (node->color[i] > 1)
                PRINTB_NOCACHE(
                    "WARNING: color() expects numbers between 0.0 and 1.0. "
                    "Value of %.1f is too large.",
                    node->color[i]);
        }
        ...
        if (webcolors.find(colorname) != webcolors.end()) {
            ...
            if (hexColor) {
                ...
                PRINTB_NOCACHE(
                    "WARNING: Unable to parse color \"%s\". Please see",
                    colorname);
                PRINT_NOCACHE(
...
std::string ColorNode::toString() const {
    ...
    stream << "color([" << this->color[0] << ", " << this->color[1] << ", "
...
void register_builtin_color() { Builtins::init("color", new ColorModule()); }
