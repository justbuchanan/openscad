...
void export_dxf(const Polygon2d &poly, std::ostream &output) {
	...
	for (const auto &o : poly.outlines()) {
		for (unsigned int i = 0; i < o.vertices.size(); i++) {
			...
			// linearly. Some libraries, like the python libraries dxfgrabber
			// and ezdxf, cannot open [X1 X2 Y1 Y2] order.
			output << "  8\n"
			       << "0\n"
			       << " 10\n"
			       << x1 << "\n"
	...
	// Some importers (e.g. Inkscape) needs an OBJECTS section with a DICTIONARY
	// entry
	output << "  0\n"
	       << "SECTION\n"
	       << "  2\n"
...
void export_dxf(const shared_ptr<const Geometry> &geom, std::ostream &output) {
	if (dynamic_cast<const PolySet *>(geom.get())) {
		assert(false && "Unsupported file format");
	               ...
	               dynamic_cast<const Polygon2d *>(geom.get())) {
		export_dxf(*poly, output);
	} else {
		assert(false &&
		       "Export as DXF for this geometry type is not supported");
	}
}
