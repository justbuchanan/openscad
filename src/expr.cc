...
Value::VectorType flatten(Value::VectorType const &vec) {
	...
	ret.reserve(n);
	for (unsigned int i = 0; i < vec.size(); i++) {
		std::copy(vec[i]->toVector().begin(), vec[i]->toVector().end(),
		          std::back_inserter(ret));
	}
	return ret;
}

void evaluate_sequential_assignment(const AssignmentList &assignment_list,
                                    Context *context) {
...
Literal::Literal(const ValuePtr &val, const Location &loc)
    : Expression(loc), value(val) {}

ValuePtr Literal::evaluate(const class Context *) const { return this->value; }

void Literal::print(std::ostream &stream) const { stream << *this->value; }
		...
		if (endValue->type() == Value::ValueType::NUMBER) {
			if (!this->step) {
				...
				if (stepValue->type() == Value::ValueType::NUMBER) {
					RangeType range(beginValue->toDouble(),
					                stepValue->toDouble(),
					                endValue->toDouble());
					return ValuePtr(range);
				}
...
void Lookup::print(std::ostream &stream) const { stream << this->name; }
...
FunctionCall::FunctionCall(const std::string &name, const AssignmentList &args,
                           const Location &loc)
...
Expression *FunctionCall::create(const std::string &funcname,
	...
	if (funcname == "assert" &&
	    Feature::ExperimentalAssertExpression.is_enabled()) {
		return new Assert(arglist, expr, loc);
	} else if (funcname == "echo" &&
	           Feature::ExperimentalEchoExpression.is_enabled()) {
...
Assert::Assert(const AssignmentList &args, Expression *expr,
               const Location &loc)
...
ValuePtr LcIf::evaluate(const Context *context) const {
	if (this->elseexpr) {
		ExperimentalFeatureException::check(
		    Feature::ExperimentalElseExpression);
	}

	const shared_ptr<Expression> &expr =
	    this->cond->evaluate(context) ? this->ifexpr : this->elseexpr;
...
ValuePtr LcFor::evaluate(const Context *context) const {
	...
	// comprehension for statements are by the parser reduced to only contain
	// one single element
...
LcForC::LcForC(const AssignmentList &args, const AssignmentList &incrargs,
               Expression *cond, Expression *expr, const Location &loc)
    : ListComprehension(loc),
      arguments(args),
      incr_arguments(incrargs),
      cond(cond),
      expr(expr) {}
...
ValuePtr LcForC::evaluate(const Context *context) const {
	...
	while (this->cond->evaluate(&c)) {
		...
		if (counter++ == 1000000)
			throw RecursionException::create("for loop", "");
