...
PolySet *Polygon2d::tessellate() const {
    ...
    for (const auto &outline : this->outlines()) {
        ...
        auto prev =
            cdt.insert({outline.vertices[outline.vertices.size() - 1][0],
                        outline.vertices[outline.vertices.size() - 1][1]});
    ...
    OPENSCAD_CGAL_ERROR_END("CGAL error in Polygon2d::tesselate()",
                            return nullptr);
