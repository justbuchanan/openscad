...
static std::string lookupResourcesPath() {
	...
	for (int a = 0; searchpath[a] != nullptr; a++) {
		...
		// The resource folder is the folder which contains "color-schemes" (as
		// well as "examples" and "locale", and optionally "libraries" and
		// "fonts")
		...
		if (is_directory(checkdir)) {
			resourcedir = tmpdir;
			PRINTDB("Found resource folder '%s'",
...
std::string PlatformUtils::applicationPath() {
	if (!path_initialized) {
		throw std::runtime_error(
		    "PlatformUtils::applicationPath(): application path not "
		    "initialized!");
	}
...
std::string PlatformUtils::resourceBasePath() {
	if (!path_initialized) {
		throw std::runtime_error(
		    "PlatformUtils::resourcesPath(): application path not "
		    "initialized!");
	}
