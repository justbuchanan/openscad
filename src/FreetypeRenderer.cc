...
double FreetypeRenderer::calc_x_offset(std::string halign, double width) const
{
	if (halign == "right") {
		...
		if (halign != "left") {
			PRINTB("Unknown value for the halign parameter (use \"left\", \"right\" or \"center\"): '%s'",
						 halign);
		}
		return 0;
	}
...
double FreetypeRenderer::calc_y_offset(std::string valign, double ascend, double descend) const
{
	if (valign == "top") {
		...
		if (valign != "baseline") {
			PRINTB("Unknown value for the valign parameter (use \"baseline\", \"bottom\", \"top\" or "
						 "\"center\"): '%s'",
						 valign);
		}
		return 0;
...
hb_direction_t FreetypeRenderer::get_direction(const FreetypeRenderer::Params &params,
