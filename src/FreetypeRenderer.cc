...
double FreetypeRenderer::calc_x_offset(std::string halign, double width) const {
	if (halign == "right") {
		...
		if (halign != "left") {
			PRINTB(
			    "Unknown value for the halign parameter (use \"left\", "
			    "\"right\" or \"center\"): '%s'",
			    halign);
		}
		return 0;
...
double FreetypeRenderer::calc_y_offset(std::string valign, double ascend,
                                       double descend) const {
	if (valign == "top") {
		return -ascend;
		...
		if (valign != "baseline") {
			PRINTB(
			    "Unknown value for the valign parameter (use \"baseline\", "
			    "\"bottom\", \"top\" or \"center\"): '%s'",
			    valign);
		}
		return 0;
	}
...
hb_direction_t FreetypeRenderer::get_direction(
    const FreetypeRenderer::Params &params, const hb_script_t script) const {
	hb_direction_t param_direction =
	    hb_direction_from_string(params.direction.c_str(), -1);
	...
	PRINTDB("Detected direction '%s' for %s",
	        hb_direction_to_string(direction) % params.text.c_str());
	return direction;
}
...
hb_script_t FreetypeRenderer::get_script(const FreetypeRenderer::Params &params,
	...
	PRINTDB("Detected script '%c%c%c%c' for %s",
	        SCRIPT_UNTAG(script) % params.text.c_str());
	return script;
}

void FreetypeRenderer::detect_properties(
    FreetypeRenderer::Params &params) const {
...
std::vector<const Geometry *> FreetypeRenderer::render(
	...
	hb_buffer_set_language(
	    hb_buf, hb_language_from_string(params.language.c_str(), -1));
	if (FontCache::instance()->is_windows_symbol_font(face)) {
		...
		if (g_utf8_validate(p, -1, nullptr)) {
			...
			PRINTB("Warning: Ignoring text with invalid UTF-8 encoding: \"%s\"",
			       params.text.c_str());
		}
	} else {
		hb_buffer_add_utf8(hb_buf, params.text.c_str(),
		                   strlen(params.text.c_str()), 0,
	...
	for (unsigned int idx = 0; idx < glyph_count; idx++) {
		...
		if (error) {
			PRINTB("Could not load glyph %u for char at index %u in text '%s'",
		...
		if (error) {
			PRINTB("Could not get glyph %u for char at index %u in text '%s'",
			       glyph_index % idx % params.text);
			continue;
		}
		const GlyphData *glyph_data =
		    new GlyphData(glyph, idx, &glyph_pos[idx]);
		glyph_array.push_back(glyph_data);
	}

	double width = 0, ascend = 0, descend = 0;
	for (GlyphArray::iterator it = glyph_array.begin(); it != glyph_array.end();
	     it++) {
	...
	for (GlyphArray::iterator it = glyph_array.begin(); it != glyph_array.end();
		...
		callback.set_glyph_offset(x_offset + glyph->get_x_offset(),
		                          y_offset + glyph->get_y_offset());
		FT_Outline outline =
