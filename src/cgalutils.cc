...
static CGAL_Nef_polyhedron *createNefPolyhedronFromPolySet(const PolySet &ps) {
    ...
    if (ps_tri.is_convex()) {
        ...
        // NB! CGAL's convex_hull_3() doesn't like std::set iterators, so we use
        // a list instead.
    ...
    try {
        ...
        if (std::string(e.what()).find("Plane_constructor") !=
                std::string::npos &&
            std::string(e.what()).find("has_on") != std::string::npos) {
            PRINT(
                "PolySet has nonplanar faces. Attempting alternate "
                "construction");
            plane_error = true;
        } else {
...
static CGAL_Nef_polyhedron *createNefPolyhedronFromPolygon2d(
...
/*!
              Check if all faces of a polyset is within 0.1 degree of being
   convex.

              NB! This function can give false positives if the polyset contains
              non-planar faces. To be on the safe side, consider passing a
   tessellated polyset. See issue #1061.
      */
bool is_approximately_convex(const PolySet &ps) {
    ...
    for (size_t i = 0; i < ps.polygons.size(); i++) {
        ...
        for (size_t j = 0; j < N; j++) {
            ...
            // edge_to_facet_map.find(other_edge);  if (it ==
            // edge_to_facet_map.end()) return false; // not a closed manifold
    ...
    while (!facets_to_visit.empty()) {
        ...
        for (size_t i = 0; i < ps.polygons[f].size(); i++) {
            ...
            auto it = edge_to_facet_map.find(
                Edge(ps.polygons[f][j], ps.polygons[f][i]));
...
CGAL_Nef_polyhedron *createNefPolyhedronFromGeometry(const Geometry &geom) {
    ...
    assert(false &&
           "createNefPolyhedronFromGeometry(): Unsupported geometry type");
    return nullptr;
}
...
*/
#if 1
bool createPolySetFromNefPolyhedron3(const CGAL_Nef_polyhedron3 &N,
                                     PolySet &ps) {
    ...
    CGAL_forall_halffacets(hfaceti, N) {
        ...
        // conversion. To avoid passing equal vertices to the tessellator, we
        // remove consecutively identical vertices.
        ...
        if (!hfaceti->incident_volume()->mark()) {
            ...
            CGAL_forall_facet_cycles_of(cyclei, hfaceti) {
                ...
                CGAL_For_all(c1, c2) {
                    ...
                    // Create vertex indices and remove consecutive duplicate
                    // vertices
    ...
    if (unconnected > 0) {
        PRINTB("Error: Non-manifold mesh encountered: %d unconnected edges",
               unconnected);
    }
    // 3. Triangulate each face
    ...
    for (const auto &faces : polygons) {
        ...
        /* at this stage, we have a sequence of polygons. the first
                 is the "outside edge' or 'body' or 'border', and the rest of
           the polygons are 'holes' within the first. there are several options
           here to get rid of the holes. we choose to go ahead and let the
           tessellater deal with the holes, and then just output the resulting
           3d triangles*/

        // We cannot trust the plane from Nef polyhedron to be correct.
        // Passing an incorrect normal vector can cause a crash in the
        // constrained delaunay triangulator See
        ...
        // K::Vector_3 normal(CGAL::to_double(nvec.x()),
        // CGAL::to_double(nvec.y()), CGAL::to_double(nvec.z()));
        std::vector<IndexedTriangle> triangles;
        auto err = GeometryUtils::tessellatePolygonWithHoles(
            verts, faces, triangles, nullptr);
        if (!err) {
            for (const auto &t : triangles) {
                assert(t[0] >= 0 &&
                       t[0] < static_cast<int>(allVertices.size()));
                assert(t[1] >= 0 &&
                       t[1] < static_cast<int>(allVertices.size()));
                assert(t[2] >= 0 &&
                       t[2] < static_cast<int>(allVertices.size()));
    ...
    if (unconnected2 > 0) {
        PRINTB(
            "Error: Non-manifold triangle mesh created: %d unconnected edges",
            unconnected2);
    }
