...
static CGAL_Nef_polyhedron *createNefPolyhedronFromPolySet(const PolySet &ps)
{
	...
	if (plane_error) try {
			...
			PRINTB("ERROR: Alternate construction failed. CGAL error in CGAL_Nef_polyhedron3(): %s",
						 e.what());
		}
...
*/
	...
	bool createPolySetFromNefPolyhedron3(const CGAL_Nef_polyhedron3 &N, PolySet &ps)
	{
		...
		for (size_t i=0;i<allVertices.size();i++) {
			...
			// Passing an incorrect normal vector can cause a crash in the constrained delaunay
			// triangulator See
