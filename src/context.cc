...
void Context::setVariables(const AssignmentList &args,
	...
	for (const auto &arg : args) {
		set_variable(arg.name, arg.expr ? arg.expr->evaluate(this->parent)
		                                : ValuePtr::undefined);
	}
...
ValuePtr Context::evaluate_function(const std::string &name,
...
AbstractNode *Context::instantiate_module(const ModuleInstantiation &inst,
...
#ifdef DEBUG
std::string Context::dump(const AbstractModule *mod,
