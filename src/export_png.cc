...
bool export_png(const shared_ptr<const Geometry> &root_geom, Camera &cam,
                std::ostream &output) {
...
#endif
    ThrownTogetherRenderer thrownTogetherRenderer(csgInfo.root_products,
...
bool export_png_with_throwntogether(Tree &tree, Camera &cam,
                                    std::ostream &output) {
    PRINTD("export_png_w_thrown");
    return export_png_preview_common(tree, cam, output,
