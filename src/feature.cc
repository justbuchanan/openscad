...
const Feature Feature::ExperimentalForCExpression(
		"lc-for-c", "Enable C-style <code>for</code> expression in list comprehensions.");
...
const Feature Feature::ExperimentalInputDriver("input-driver",
																							 "Enable input drivers (requires restart)");
...
void Feature::dump_features()
{
	for (map_t::iterator it = feature_map.begin(); it != feature_map.end(); it++) {
		std::cout << "Feature('" << it->first
							<< "') = " << (it->second->is_enabled() ? "enabled" : "disabled") << std::endl;
	}
}
...
void ExperimentalFeatureException::check(const Feature &feature)
{
	if (!feature.is_enabled()) {
		...
		out << "ERROR: Experimental feature not enabled: '" << feature.get_name()
