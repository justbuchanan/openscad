 ...
 * context.
 */
const Feature Feature::ExperimentalAssertExpression(
    "assert", "Enable <code>assert</code>.");
const Feature Feature::ExperimentalEchoExpression(
    "echo", "Enable <code>echo</code> expression.");
const Feature Feature::ExperimentalEachExpression(
    "lc-each", "Enable <code>each</code> expression in list comprehensions.");
const Feature Feature::ExperimentalElseExpression(
    "lc-else", "Enable <code>else</code> expression in list comprehensions.");
const Feature Feature::ExperimentalForCExpression(
    "lc-for-c",
    "Enable C-style <code>for</code> expression in list comprehensions.");
const Feature Feature::ExperimentalAmfImport("amf-import",
                                             "Enable AMF import.");
const Feature Feature::ExperimentalSvgImport("svg-import",
                                             "Enable SVG import.");
const Feature Feature::ExperimentalCustomizer("customizer",
                                              "Enable Customizer");
...
bool Feature::is_enabled() const { return enabled; }
...
void Feature::enable_feature(const std::string &feature_name, bool status) {
	...
	if (it != feature_map.end()) {
		...
		PRINTB("WARNING: Ignoring request to enable unknown feature '%s'.",
		       feature_name);
	}
}
...
void Feature::dump_features() {
	for (map_t::iterator it = feature_map.begin(); it != feature_map.end();
	     it++) {
		std::cout << "Feature('" << it->first << "') = "
		          << (it->second->is_enabled() ? "enabled" : "disabled")
		          << std::endl;
	}
}
...
void ExperimentalFeatureException::check(const Feature &feature) {
	if (!feature.is_enabled()) {
		...
		out << "ERROR: Experimental feature not enabled: '"
