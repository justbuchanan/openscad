...
namespace /* anonymous */ {
	...
	class CGAL_Build_PolySet : public CGAL::Modifier_base<typename Polyhedron::HalfedgeDS>
	{
		...
		typedef CGAL::Polyhedron_incremental_builder_3<typename Polyhedron::HalfedgeDS>
				CGAL_Polybuilder;
...
*/
	...
	struct Copy_polyhedron_to : public CGAL::Modifier_base<typename Polyhedron_output::HalfedgeDS> {
		...
		void operator()(typename Polyhedron_output::HalfedgeDS &out_hds)
		{
			...
			for (Vertex_const_iterator vi = in_poly.vertices_begin(), end = in_poly.vertices_end();
					 vi != end; ++vi) {
			...
			for (Facet_const_iterator fi = in_poly.facets_begin(), end = in_poly.facets_end(); fi != end;
					 ++fi) {
