...
class CGAL_Build_PolySet
    ...
    typedef CGAL::Polyhedron_incremental_builder_3<
...
/*
  ...
  model. If we don't grid the geometry before converting to a Nef Polyhedron,
  the quads in the cylinders to tessellated into triangles since floating point
    ...
    void operator()(HDS &hds) {
        ...
        for (const auto &p : ps.polygons) {
            ...
            for (auto v : boost::adaptors::reverse(p)) {
                // align v to the grid; the CGALPoint will receive the aligned
                // vertex
        ...
        for (auto &pindices : indices) {
            ...
            // Polyhedron_incremental_builder_3::test_facet() which fails to
            // detect this
            std::vector<size_t>::iterator last =
                std::unique(pindices.begin(), pindices.end());
            std::advance(last, -1);
    ...
    void operator()(HDS &hds) {
        ...
        for (const auto &p : ps.polygons) {
            ...
            for (std::size_t i = 0; i < indices.size(); ++i) {
                // check if vertex indices[i] is already in the sequence
                // [0..i-1]
...
template void copyPolyhedron(const CGAL::Polyhedron_3<CGAL::Epick> &,
                             CGAL_Polyhedron &);
template void copyPolyhedron(const CGAL_Polyhedron &,
...
bool createPolyhedronFromPolySet(const PolySet &ps, Polyhedron &p) {
    ...
    try {
        ...
        PRINTB("CGAL error in CGALUtils::createPolyhedronFromPolySet: %s",
               e.what());
        err = true;
    }
...
template bool createPolySetFromPolyhedron(const CGAL_Polyhedron &p,
                                          PolySet &ps);
template bool createPolySetFromPolyhedron(
    const CGAL::Polyhedron_3<CGAL::Epick> &p, PolySet &ps);
template bool createPolySetFromPolyhedron(
    const CGAL::Polyhedron_3<CGAL::Epeck> &p, PolySet &ps);
template bool createPolySetFromPolyhedron(
    const CGAL::Polyhedron_3<CGAL::Simple_cartesian<long>> &p, PolySet &ps);
