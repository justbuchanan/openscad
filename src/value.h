...
public:
    ...
    enum class ValueType { UNDEFINED, BOOL, NUMBER, STRING, VECTOR, RANGE };
    ...
    bool getVec3(double &x, double &y, double &z,
    ...
    typedef boost::variant<boost::blank, bool, double, std::string, VectorType,
                           RangeType>
        Variant;

private:
    static Value multvecnum(const Value &vecval, const Value &numval);
    static Value multmatvec(const VectorType &matrixvec,
                            const VectorType &vectorvec);
    static Value multvecmat(const VectorType &vectorvec,
