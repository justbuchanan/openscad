...
static Polygon2d *convertToPolygon2d(const CGAL_Nef_polyhedron2 &p2) {
	...
	for (fci_t fit = E.faces_begin(), facesend = E.faces_end(); fit != facesend;
		...
		for (CGAL_Nef_polyhedron2::Explorer::Hole_const_iterator j =
		         E.holes_begin(fit);
		     j != E.holes_end(fit); ++j) {
			CGAL_Nef_polyhedron2::Explorer::
			    Halfedge_around_face_const_circulator hcirc(j),
			    hend(hcirc);
...
void ZRemover::visit(CGAL_Nef_polyhedron3::Halffacet_const_handle hfacet) {
	...
	CGAL_forall_facet_cycles_of(fci, hfacet) {
		if (fci.is_shalfedge()) {
			...
			CGAL_Nef_polyhedron3::SHalfedge_around_facet_const_circulator c1(
			    fci),
			    cend(c1);
			std::vector<CGAL_Nef_polyhedron2::Explorer::Point> contour;
			CGAL_For_all(c1, cend) {
				CGAL_Nef_polyhedron3::Point_3 point3d =
				    c1->source()->target()->point();
			...
			tmpnef2d.reset(new CGAL_Nef_polyhedron2(contour.begin(),
			                                        contour.end(), boundary));
			...
			if (contour_counter == 0) {
				...
				*(output_nefpoly2d) += *(tmpnef2d);
				...
				PRINTDB(
				    " <!-- contour is a hole. make intersection(). %i  points "
				    "-->",
				    contour.size());
				*(output_nefpoly2d) *= *(tmpnef2d);
			}
...
Polygon2d *project(const CGAL_Nef_polyhedron &N, bool cut) {
	...
	if (cut) {
		...
		try {
			...
			newN.p3.reset(new CGAL_Nef_polyhedron3(N.p3->intersection(
			    xy_plane, CGAL_Nef_polyhedron3::PLANE_ONLY)));
		} catch (const CGAL::Failure_exception &e) {
			PRINTDB("CGALUtils::project during plane intersection: %s",
			        e.what());
			try {
				PRINTD(
				    "Trying alternative intersection using very large thin "
				    "box: ");
				...
				newN.p3.reset(
				    new CGAL_Nef_polyhedron3(nef_bigbox.intersection(*N.p3)));
			} catch (const CGAL::Failure_exception &e) {
				PRINTB(
				    "ERROR: CGAL error in CGALUtils::project during bigbox "
				    "intersection: %s",
				    e.what());
			}
		}
		...
		try {
			...
			for (i = newN.p3->volumes_begin(); i != newN.p3->volumes_end();
			     ++i) {
			...
			PRINTB(
			    "ERROR: CGAL error in CGALUtils::project while flattening: %s",
			    e.what());
		}
		PRINTD("</svg>");
