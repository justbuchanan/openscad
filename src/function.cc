...
UserFunction::UserFunction(const char *name, AssignmentList &definition_arguments,
...
public:
	FunctionTailRecursion(const char *name, AssignmentList &definition_arguments,
												...
												shared_ptr<Expression> endexpr, bool invert, const Location &loc)
																	 ...
																	 shared_ptr<Expression> expr, const Location &loc)
{
	if (shared_ptr<TernaryOp> ternary = dynamic_pointer_cast<TernaryOp>(expr)) {
		...
		if (ifcall && !elsecall) {
			if (name == ifcall->name) {
				return new FunctionTailRecursion(name, definition_arguments, ternary, ifcall,
																				 ternary->elseexpr, false, loc);
			}
		} else if (elsecall && !ifcall) {
			if (name == elsecall->name) {
				return new FunctionTailRecursion(name, definition_arguments, ternary, elsecall,
