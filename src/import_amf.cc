...
void AmfImporter::set_v1(AmfImporter *importer, const xmlChar *value) {
	importer->idx_v1 =
	    boost::lexical_cast<int>(std::string((const char *)value));
}
...
void AmfImporter::end_vertex(AmfImporter *importer, const xmlChar *) {
	...
	importer->vertex_list.push_back(
	    Eigen::Vector3d(importer->x, importer->y, importer->z));
}
...
void AmfImporter::end_triangle(AmfImporter *importer, const xmlChar *) {
	...
	PRINTDB("AMF: add triangle %d - (%.2f, %.2f, %.2f)",
	        importer->vertex_list.size() % idx_v1 % idx_v2 % idx_v3);

	std::vector<Eigen::Vector3d> &v = importer->vertex_list;

	importer->polySet->append_poly();
	importer->polySet->append_vertex(v[idx_v1].x(), v[idx_v1].y(),
	                                 v[idx_v1].z());
	importer->polySet->append_vertex(v[idx_v2].x(), v[idx_v2].y(),
	                                 v[idx_v2].z());
	importer->polySet->append_vertex(v[idx_v3].x(), v[idx_v3].y(),
	                                 v[idx_v3].z());
}
...
PolySet *AmfImporter::read(const std::string filename) {
	...
	if (polySets.size() > 1) {
		...
		for (std::vector<PolySet *>::iterator it = polySets.begin();
		     it != polySets.end(); it++) {
			children.push_back(std::make_pair((const AbstractNode *)nullptr,
			                                  shared_ptr<const Geometry>(*it)));
		}
		CGAL_Nef_polyhedron *N =
		    CGALUtils::applyOperator(children, OpenSCADOperator::UNION);
		PolySet *result = new PolySet(3);
		if (CGALUtils::createPolySetFromNefPolyhedron3(*N->p3, *result)) {
			delete result;
			p = new PolySet(3);
			PRINTB("ERROR: Error importing multi-object AMF file '%s'",
			       filename);
		} else {
			p = result;
		}
...
xmlTextReaderPtr AmfImporterZIP::createXmlReader(const char *filename) {
	...
	if (archive) {
		...
		if (zipfile == nullptr) {
			PRINTB("WARNING: Can't read file '%s' from zipped AMF '%s'",
			       f.filename().c_str() % filename);
		}
		if ((zipfile == nullptr) && (zip_get_num_files(archive) == 1)) {
			PRINTB("WARNING: Trying to read single entry '%s'",
			       zip_get_name(archive, 0, 0));
			zipfile = zip_fopen_index(archive, 0, 0);
		}
		if (zipfile) {
			return xmlReaderForIO(
			    read_callback, close_callback, this, f.filename().c_str(),
			    nullptr,
			    XML_PARSE_NOENT | XML_PARSE_NOERROR | XML_PARSE_NOWARNING);
		} else {
			zip_close(archive);
