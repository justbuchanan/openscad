...
public:
	...
	public:
		...
		void set_direction(std::string direction) { this->direction = direction; }
		...
		friend std::ostream &operator<<(std::ostream &stream, const FreetypeRenderer::Params &params)
		{
			return stream << "text = \"" << params.text << "\", size = " << params.size
										...
										<< "\", direction = \"" << params.direction << "\", language = \""
										<< params.language << (params.script.empty() ? "" : "\", script = \"")
										<< params.script << "\", halign = \"" << params.halign << "\", valign = \""
										<< params.valign << "\", $fn = " << params.fn << ", $fa = " << params.fa
										<< ", $fs = " << params.fs;
		}
...
private:
	...
	hb_script_t get_script(const FreetypeRenderer::Params &params, hb_glyph_info_t *glyph_info,
