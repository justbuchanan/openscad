...
void shape::set_attrs(attr_map_t& attrs) {
	this->id = attrs["id"];
...
std::string shape::get_style(std::string name) {
	std::vector<std::string> styles;
	boost::split(styles, this->style, boost::is_any_of(";"));

	for (std::vector<std::string>::iterator it = styles.begin();
	     it != styles.end(); it++) {
...
ClipperLib::EndType shape::get_stroke_linecap() {
	std::string cap;
...
void shape::collect_transform_matrices(std::vector<Eigen::Matrix3d>& matrices,
	...
	for (std::vector<transformation*>::reverse_iterator it =
	         transformations.rbegin();
	     it != transformations.rend(); it++) {
		transformation* t = *it;
...
void shape::apply_transform() {
	...
	for (path_list_t::iterator it = path_list.begin(); it != path_list.end();
	     it++) {
		path_t& p = *it;
		...
		for (path_t::iterator it2 = p.begin(); it2 != p.end(); it2++) {
			...
			for (std::vector<Eigen::Matrix3d>::reverse_iterator it3 =
			         matrices.rbegin();
			     it3 != matrices.rend(); it3++) {
				result = *it3 * result;
			}
...
void shape::offset_path(path_list_t& path_list, path_t& path,
                        double stroke_width,
                        ClipperLib::EndType stroke_linecap) {
	...
	for (ClipperLib::Paths::iterator it = result.begin(); it != result.end();
	     it++) {
		ClipperLib::Path& p = *it;
		path_list.push_back(path_t());
		for (ClipperLib::Path::iterator it2 = p.begin(); it2 != p.end();
		     it2++) {
			ClipperLib::IntPoint& point = *it2;
			path_list.back().push_back(
			    Eigen::Vector3d(point.X / 10000.0, point.Y / 10000.0, 0));
		}
		path_list.back().push_back(
		    Eigen::Vector3d(p[0].X / 10000.0, p[0].Y / 10000.0, 0));
	}
}

void shape::draw_ellipse(path_t& path, double x, double y, double rx,
                         double ry) {
	unsigned long fn = 40;
...
std::ostream& operator<<(std::ostream& os, const shape& s) {
	return os << s.get_name() << " | id = '" << s.id << "', transform = '"
	          << s.transform << "'";
}
