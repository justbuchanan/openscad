...
static void help(const char *progname, bool failure = false) {
    ...
    PRINTB(
        ...
        "%2%[ "
        "--colorscheme=[Cornfield|Sunset|Metallic|Starnight|BeforeDawn|Nature|"
        "DeepOcean] ] \\\n"
        "%2%[ --csglimit=num ]"
...
Camera get_camera(po::variables_map vm) {
    ...
    if (vm.count("projection")) {
        ...
        if (proj == "o" || proj == "ortho" || proj == "orthogonal") {
            ...
            PRINT(
                "projection needs to be 'o' or 'p' for ortho or perspective\n");
            exit(1);
        }
    }
...
void set_render_color_scheme(const std::string color_scheme,
    ...
    if (exit_if_not_found) {
        ...
        PRINTB("Unknown color scheme '%s', using default '%s'.",
               arg_colorscheme % ColorMap::inst()->defaultColorSchemeName());
    }
}

#include <QCoreApplication>

int cmdline(const char *deps_output_file, const std::string &filename,
            Camera &camera, const char *output_file,
            const fs::path &original_path, RenderType renderer,
            const std::string &parameterFile, const std::string &setName,
            int argc, char **argv) {
    ...
    const std::string application_path =
        fs::absolute(boost::filesystem::path(argv[0]).parent_path())
            .generic_string();
#endif
    ...
    if (suffix == ".stl")
        stl_output_file = output_file;
    else if (suffix == ".off")
        off_output_file = output_file;
    else if (suffix == ".amf")
        amf_output_file = output_file;
    else if (suffix == ".dxf")
        dxf_output_file = output_file;
    else if (suffix == ".svg")
        svg_output_file = output_file;
    else if (suffix == ".csg")
        csg_output_file = output_file;
    else if (suffix == ".png")
        png_output_file = output_file;
    else if (suffix == ".ast")
        ast_output_file = output_file;
    else if (suffix == ".term")
        term_output_file = output_file;
    else if (suffix == ".echo")
        echo_output_file = output_file;
    else if (suffix == ".nefdbg")
        nefdbg_output_file = output_file;
    else if (suffix == ".nef3")
        nef3_output_file = output_file;
    else {
    ...
    bool preview = png_output_file ? (renderer == RenderType::OPENCSG ||
    ...
    absolute_root_node =
        root_module->instantiate(&top_ctx, &root_inst, nullptr);
    ...
    if (csg_output_file) {
        ...
        if (!fstream.is_open()) {
            ...
            fs::current_path(fparent);  // Force exported filenames to be
        ...
        if (stl_output_file) {
            if (!checkAndExport(root_geom, 3, FileFormat::STL,
                                stl_output_file)) {
                return 1;
            }
        }
        ...
        if (off_output_file) {
            if (!checkAndExport(root_geom, 3, FileFormat::OFF,
                                off_output_file)) {
                return 1;
            }
        }
        ...
        if (amf_output_file) {
            if (!checkAndExport(root_geom, 3, FileFormat::AMF,
                                amf_output_file)) {
                return 1;
            }
        }
        ...
        if (dxf_output_file) {
            if (!checkAndExport(root_geom, 2, FileFormat::DXF,
                                dxf_output_file)) {
                return 1;
            }
        }
        ...
        if (svg_output_file) {
            if (!checkAndExport(root_geom, 2, FileFormat::SVG,
                                svg_output_file)) {
                return 1;
            }
        }
        ...
        if (nefdbg_output_file) {
            if (!checkAndExport(root_geom, 3, FileFormat::NEFDBG,
                                nefdbg_output_file)) {
                return 1;
            }
        }
        ...
        if (nef3_output_file) {
            if (!checkAndExport(root_geom, 3, FileFormat::NEF3,
                                nef3_output_file)) {
                return 1;
            }
        }
...
static QString assemblePath(const fs::path &absoluteBaseDir,
    ...
    auto qsDir =
        QString::fromLocal8Bit(absoluteBaseDir.generic_string().c_str());
...
void dialogInitHandler(FontCacheInitializer *initializer, void *) {
    ...
    QObject::connect(&futureWatcher, SIGNAL(finished()), scadApp,
                     SLOT(hideFontCacheDialog()));
    ...
    // Block, in case we're in a separate thread, or the dialog was closed by
    // the user
...
int gui(vector<string> &inputFiles, const fs::path &original_path, int argc,
        char **argv) {
#ifdef Q_OS_MACX
    ...
    auto appPath = QDir::toNativeSeparators(app.applicationFilePath() +
                                            QLatin1String(",1"));
    ...
    // This workaround appears to only be needed when QGLWidget is used
    // QOpenGLWidget available in Qt 5.4 is much better.
    QGLFormat fmt;
...
int gui(const vector<string> &inputFiles, const fs::path &original_path,
        int argc, char **argv) {
...
int main(int argc, char **argv) {
    ...
    // (which we don't catch). This gives us stack traces without rerunning in
    // gdb.
    ...
    desc.add_options()("help,h", "help message")("version,v",
                                                 "print the version")(
        "info", "print information about the building process")(
        "render", po::value<string>()->implicit_value(""),
        "if exporting a png image, do a full geometry evaluation")(
        "preview", po::value<string>()->implicit_value(""),
        "if exporting a png image, do an OpenCSG(default) or ThrownTogether "
        "preview")("csglimit", po::value<unsigned int>(),
                   "if exporting a png image, stop rendering at the given "
                   "number of CSG elements")(
        "camera", po::value<string>(),
        "parameters for camera when exporting png")(
        "autocenter", "adjust camera to look at object center")(
        "viewall", "adjust camera to fit object")(
        "imgsize", po::value<string>(), "=width,height for exporting png")(
        "projection", po::value<string>(),
        "(o)rtho or (p)erspective when exporting png")(
        "colorscheme", po::value<string>(), "colorscheme")(
        "debug", po::value<string>(), "special debug info")(
        "quiet,q", "quiet mode (don't print anything *except* errors)")(
        "o,o", po::value<string>(), "out-file")("p,p", po::value<string>(),
                                                "parameter file")(
        "P,P", po::value<string>(), "parameter set")(
        "s,s", po::value<string>(), "stl-file")("x,x", po::value<string>(),
                                                "dxf-file")(
        "d,d", po::value<string>(), "deps-file")("m,m", po::value<string>(),
                                                 "makefile")(
    ...
    if (arg_info || cmdlinemode) {
        ...
        rc = cmdline(deps_output_file, inputFiles[0], camera, output_file,
                     original_path, renderer, parameterFile, parameterSet, argc,
                     argv);
    } else if (QtUseGUI()) {
