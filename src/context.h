...
public:
	...
	virtual class AbstractNode *instantiate_module(
	    const class ModuleInstantiation &inst, EvalContext *evalctx) const;

	void setVariables(const AssignmentList &args,
	                  const class EvalContext *evalctx = nullptr);
...
protected:
	...
	std::string
	    document_path;  // FIXME: This is a remnant only needed by dxfdim
