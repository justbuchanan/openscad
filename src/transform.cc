...
AbstractNode *TransformModule::instantiate(const Context *ctx,
                                           const ModuleInstantiation *inst,
                                           EvalContext *evalctx) const {
    ...
    if (this->type == transform_type_e::SCALE) {
        ...
        if (x != 0.0 || y != 0.0 || z != 0.0) {
            Matrix4d m;
            m << 1 - 2 * x * x, -2 * y * x, -2 * z * x, 0, -2 * x * y,
                1 - 2 * y * y, -2 * z * y, 0, -2 * x * z, -2 * y * z,
                1 - 2 * z * z, 0, 0, 0, 0, 1;
            node->matrix = m;
        }
        ...
        if (v->type() == Value::ValueType::VECTOR) {
            ...
            for (int i = 0; i < 16; i++) {
                size_t x = i / 4, y = i % 4;
                if (y < v->toVector().size() &&
                    v->toVector()[y]->type() == Value::ValueType::VECTOR &&
...
void register_builtin_transform() {
    ...
    Builtins::init("translate",
                   new TransformModule(transform_type_e::TRANSLATE));
    Builtins::init("multmatrix",
                   new TransformModule(transform_type_e::MULTMATRIX));
}
