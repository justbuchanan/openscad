...
static bool check_valid(const fs::path &p,
	...
	if (openfilenames) {
		for (const auto &s : *openfilenames) {
			if (s == fullname) {
				//				PRINTB("WARNING: circular include file %s",
				// fullname);
				return false;
			}
		}
...
fs::path find_valid_path(const fs::path &sourcepath, const fs::path &localpath,
	...
	if (localpath.is_absolute()) {
		if (check_valid(localpath, openfilenames))
			return boosty::canonical(localpath);
	} else {
...
void parser_init() {
	...
	if (openscadpaths) {
		...
		for (string_split_iterator it = boost::make_split_iterator(
			...
			add_librarydir(
			    fs::absolute(fs::path(boost::copy_range<std::string>(*it)))
			        .string());
		}
	}
	...
	add_librarydir(
	    fs::absolute(PlatformUtils::resourcePath("libraries")).string());
}
