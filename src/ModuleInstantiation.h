...
public:
    ...
    std::vector<AbstractNode *> instantiateChildren(
...
public:
    IfElseModuleInstantiation(shared_ptr<class Expression> expr,
                              const std::string &source_path,
                              const Location &loc)
        : ModuleInstantiation("if", AssignmentList{Assignment("", expr)},
                              source_path, loc) {}
    virtual ~IfElseModuleInstantiation();
    std::vector<AbstractNode *> instantiateElseChildren(
        const Context *evalctx) const;
    virtual std::string dump(const std::string &indent) const;
