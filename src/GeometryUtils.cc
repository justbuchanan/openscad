...
public:
    ...
    typedef std::unordered_map<IndexedEdge, int, boost::hash<IndexedEdge>>
        IndexedEdgeDict;
    ...
    void print() const {
        for (const auto &v : this->edges) {
            ...
            PRINTDB(
                ...
                e.first % e.second %
                    ((v.second > 1)
                         ? boost::lexical_cast<std::string>(v.second).c_str()
                         : ""));
        }
    }
...
bool GeometryUtils::tessellatePolygonWithHoles(
    const Vector3f *vertices, const std::vector<IndexedFace> &faces,
    ...
    for (auto &face : cleanfaces) {
        ...
        while (face.size() >= 3 && i < face.size()) {
            if (face[i] ==
                face[(i + 1) %
                     face.size()]) {  // Two consecutively equal indices
                ...
                // Filter away inf and nan vertices as they cause libtess2 to
                // crash
    ...
    // Since libtess2's indices is based on the running number of points added,
    // we need to map back to our indices. allindices does the mapping.
    ...
    /*
            ...
            C) For each unused vertex, create a triangle connecting it to the
       existing mesh
    */
    auto inputSize =
    ...
    for (int t = 0; t < numelems; t++) {
        ...
        PRINTDB("%d (%d) %d (%d) %d (%d)",
                elements[t * 3 + 0] % mappedtri[0] % elements[t * 3 + 1] %
                    mappedtri[1] % elements[t * 3 + 2] % mappedtri[2]);
        // FIXME: We ignore self-intersecting triangles rather than detecting
        // and handling this
        if (!err) {
            vflags[tri[0]]++;  // B)
            vflags[tri[1]]++;
            ...
            // For each edge in mappedtri, locate the opposite edge in the
            // original polygon. If an opposite edge was found, we need to flip.
            // In addition, remove each edge from the dict to be able to later
            // find missing edges. Note: In some degenerate cases, we create
            // triangles with mixed edge directions. In this case, don't
            // reverse, but attempt to carry on
            auto reverse = false;
            for (int i = 0; i < 3; i++) {
    ...
    if (!edges.empty()) {
        ...
        if (!edges.empty()) {
            PRINTDB("   %d edges remaining after loop triangulation",
                    edges.size());
            edges.print();
        }
    }
