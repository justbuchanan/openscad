...
bool fbo_ext_init(fbo_t *fbo, size_t width, size_t height) {
    ...
    if (!check_fbo_status()) {
        cerr << "Problem with OpenGL EXT framebuffer after specifying color "
                "render buffer.\n";
        return false;
    }
    ...
    if (glewIsSupported("GL_EXT_packed_depth_stencil")) {
        ...
        glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT,
                                     ...
                                     GL_RENDERBUFFER_EXT, fbo->depthbuf_id);
        if (report_glerror("specifying stencil render buffer EXT"))
            return false;

        if (!check_fbo_status()) {
            cerr << "Problem with OpenGL EXT framebuffer after specifying "
                    "depth render buffer.\n";
            return false;
        }
    } else {
        cerr << "Warning: Cannot create stencil buffer "
...
bool fbo_arb_init(fbo_t *fbo, size_t width, size_t height) {
    ...
    if (!check_fbo_status()) {
        cerr << "Problem with OpenGL framebuffer after specifying color render "
                "buffer.\n";
        return false;
    }
    ...
    if (!check_fbo_status()) {
        cerr << "Problem with OpenGL framebuffer after specifying depth render "
                "buffer.\n";
        return false;
    }
...
bool fbo_init(fbo_t *fbo, size_t width, size_t height) {
    ...
    by deciding at runtime, using GLEW, which version needs to be used. See
    also:
...
bool fbo_resize(fbo_t *fbo, size_t width, size_t height) {
    if (use_ext()) {
        ...
        if (glewIsSupported("GL_EXT_packed_depth_stencil")) {
            glRenderbufferStorageEXT(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8,
                                     width, height);
            if (report_glerror("creating EXT depth stencil render buffer"))
                return false;
        } else {
            glRenderbufferStorageEXT(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24,
                                     width, height);
            if (report_glerror("creating EXT depth render buffer"))
                return false;
        }
        ...
        glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width,
                              height);
        if (report_glerror("creating depth stencil render buffer"))
            return false;
    }
...
void fbo_delete(fbo_t *fbo) { delete fbo; }

GLuint fbo_bind(fbo_t *fbo) {
    glGetIntegerv(GL_FRAMEBUFFER_BINDING,
                  reinterpret_cast<GLint *>(&fbo->old_fbo_id));
    if (use_ext()) {
