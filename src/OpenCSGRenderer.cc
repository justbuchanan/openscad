...
OpenCSGRenderer::OpenCSGRenderer(shared_ptr<CSGProducts> root_products,
																 ...
																 shared_ptr<CSGProducts> background_products, GLint *shaderinfo)
	: root_products(root_products), highlights_products(highlights_products),
																				...
																				bool highlight_mode, bool background_mode) const
{
	...
	for (const auto &product : products.products) {
		...
		for (const auto &csgobj : product.intersections) {
			if (csgobj.leaf->geom)
				primitives.push_back(createCSGPrimitive(csgobj, OpenCSG::Intersection, highlight_mode,
																								background_mode, OpenSCADOperator::INTERSECTION));
		}
		for (const auto &csgobj : product.subtractions) {
			if (csgobj.leaf->geom)
				primitives.push_back(createCSGPrimitive(csgobj, OpenCSG::Subtraction, highlight_mode,
