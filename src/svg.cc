...
namespace OpenSCAD {
	...
	*/
			...
			for (sf = S.sfaces_begin(); sf != S.sfaces_end(); sf++) {
				...
				for (it = sf->sface_cycles_begin(); it != sf->sface_cycles_end(); it++) {
					if (it.is_svertex())
						...
						out << "  shalfloop lying in the plane " << SHalfloop_const_handle(it)->circle()
								<< std::endl;
					else
						out << "  unknown bug\n";
				}
	...
	// This uses the Shell Explorer pattern from the CGAL Manual to dump the 3d Nef Polyhedron
	// information
