...
std::string svg_axes() {
	std::stringstream out;
	out << " <!-- axes -->\n";
	out << "  <polyline points='10,455 10,475 10,465 18,465 2,465 10,465 "
	       "14,461 6,469 10,465'"
	    << " style='fill:none;stroke:black;' />\n";
	out << " <!-- /axes -->";
	return out.str();
...
CGAL_Nef_polyhedron2::Explorer::Point project_svg_3to2(CGAL_Point_3 p,
...
CGAL_Point_2e project_svg_2to2(const CGAL_Point_2e &p,
	...
	double ty =
	    screenh - borderh - (yinbox / (bboxh == 0 ? 1 : bboxh)) * (vizh);
	return {tx, ty};
}
...
std::string dump_cgal_nef_polyhedron2_face_svg(
    ...
    CGAL_Nef_polyhedron2::Explorer explorer, bool facemark, bool body) {
...
std::string dump_svg(const CGAL_Nef_polyhedron2 &N) {
	...
	for (auto i = explorer.faces_begin(); i != explorer.faces_end(); ++i) {
		...
		for (auto j = explorer.holes_begin(i); j != explorer.holes_end(i);
		     ++j) {
			out << "   <!-- hole begin. mark: " << j->mark() << " -->\n";
			CGAL_Nef_polyhedron2::Explorer::
			    Halfedge_around_face_const_circulator c3(j),
			    c4(c3);
			out << dump_cgal_nef_polyhedron2_face_svg(c3, c4, explorer, "green",
			                                          j->mark());
			out << "   <!-- hole end -->\n";
		}
		out << "  <!-- face end -->\n";
...
std::string sphere_map_dump(const CGAL_Nef_polyhedron3 &N) {
	...
	for (v = N.vertices_begin(); v != N.vertices_end(); v++) {
		...
		out << "  number of svertices: " << S.number_of_svertices()
		    << std::endl;
		out << "  number of shalfedges: " << S.number_of_shalfedges()
		    << std::endl;
		out << "  number of shalfloops: " << S.number_of_shalfloops()
		    << std::endl;
		...
		for (sf = S.sfaces_begin(); sf != S.sfaces_end(); sf++) {
			...
			for (it = sf->sface_cycles_begin(); it != sf->sface_cycles_end();
			     it++) {
				if (it.is_svertex())
					...
					out << "  shalfedge from "
					    << vert_dump(SHalfedge_const_handle(it)->source())
					    << " to "
					    << vert_dump(SHalfedge_const_handle(it)->target())
					    << std::endl;
	...
	void visit(CGAL_Nef_polyhedron3::Halffacet_const_handle hfacet) {
		...
		CGAL_forall_facet_cycles_of(i, hfacet) {
			...
			CGAL_Nef_polyhedron3::SHalfedge_around_facet_const_circulator c1(i),
			    c2(c1);
			CGAL_For_all(c1, c2) {
				// don't know why we use source()->source(), except thats what
				// CGAL does internally
