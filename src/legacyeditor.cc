...
LegacyEditor::LegacyEditor(QWidget *parent) : EditorInterface(parent) {
	...
	connect(this->textedit, SIGNAL(textChanged()), this,
...
QString LegacyEditor::toPlainText() { return this->textedit->toPlainText(); }
...
void LegacyEditor::undo() { this->textedit->undo(); }

void LegacyEditor::redo() { this->textedit->redo(); }

void LegacyEditor::cut() { this->textedit->cut(); }

void LegacyEditor::copy() { this->textedit->copy(); }

void LegacyEditor::paste() { this->textedit->paste(); }

LegacyEditor::~LegacyEditor() { delete highlighter; }
...
void LegacyEditor::replaceAll(const QString &findText,
...
bool LegacyEditor::findString(const QString &exp, bool findBackwards) const {
	return this->textedit->find(exp, findBackwards
	                                     ? QTextDocument::FindBackward
	                                     : QTextDocument::FindFlags(0));
}

int LegacyEditor::resetFindIndicators(
...
bool LegacyEditor::find(const QString &newText, bool /*findNext*/,
                        bool findBackwards) {
	...
	if (!success) {  // Implement wrap-around search behavior
		...
		tmp_cursor.movePosition(findBackwards ? QTextCursor::End
		                                      : QTextCursor::Start);
