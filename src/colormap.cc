...
RenderColorScheme::RenderColorScheme() : _path("") {
	...
	_color_scheme.insert(ColorScheme::value_type(
	    RenderColor::CGAL_FACE_2D_COLOR, Color4f(0x00, 0xbf, 0x99)));
	_color_scheme.insert(ColorScheme::value_type(
	...
	_color_scheme.insert(ColorScheme::value_type(
	    RenderColor::CGAL_EDGE_2D_COLOR, Color4f(0xff, 0x00, 0x00)));
...
RenderColorScheme::RenderColorScheme(fs::path path) : _path(path) {
	try {
		...
		PRINTB("Error reading color scheme file '%s': %s",
...
void RenderColorScheme::addColor(RenderColor colorKey, std::string key) {
	...
	if ((color.length() == 7) && (color.at(0) == '#')) {
		...
		_color_scheme.insert(
		    ColorScheme::value_type(colorKey, Color4f(r, g, b)));
	} else {
		throw std::invalid_argument(
		    std::string("invalid color value for key '") + key + "': '" +
		    color + "'");
	}
}
...
const ColorScheme *ColorMap::findColorScheme(const std::string &name) const {
	for (colorscheme_set_t::const_iterator it = colorSchemeSet.begin();
	     it != colorSchemeSet.end(); it++) {
...
void ColorMap::enumerateColorSchemesInPath(colorscheme_set_t &result_set,
	...
	if (fs::exists(color_schemes) && fs::is_directory(color_schemes)) {
		for (fs::directory_iterator dir_iter(color_schemes);
			...
			if (colorScheme->valid() &&
			    (findColorScheme(colorScheme->name()) == 0)) {
				result_set.insert(colorscheme_set_t::value_type(
				    colorScheme->index(),
				    shared_ptr<RenderColorScheme>(colorScheme)));
