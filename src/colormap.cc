...
RenderColorScheme::RenderColorScheme() : _path("")
{
	...
	_color_scheme.insert(
			ColorScheme::value_type(RenderColor::CROSSHAIR_COLOR, Color4f(0x80, 0x00, 0x00)));
}
...
void RenderColorScheme::addColor(RenderColor colorKey, std::string key)
{
	...
	if ((color.length() == 7) && (color.at(0) == '#')) {
		...
		throw std::invalid_argument(std::string("invalid color value for key '") + key + "': '" +
																color + "'");
	}
}
...
{
	for (colorscheme_set_t::const_iterator it = colorSchemeSet.begin(); it != colorSchemeSet.end();
			 it++) {
		...
		for (fs::directory_iterator dir_iter(color_schemes); dir_iter != end_iter; ++dir_iter) {
			...
			if (colorScheme->valid() && (findColorScheme(colorScheme->name()) == 0)) {
				result_set.insert(colorscheme_set_t::value_type(
						colorScheme->index(), shared_ptr<RenderColorScheme>(colorScheme)));
...
ColorMap::colorscheme_set_t ColorMap::enumerateColorSchemes()
{
	...
	result_set.insert(colorscheme_set_t::value_type(
			defaultColorScheme->index(), shared_ptr<RenderColorScheme>(defaultColorScheme)));
