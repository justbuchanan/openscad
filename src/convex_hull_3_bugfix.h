...
template <class Point_3,
          class Is_floating_point = typename boost::is_floating_point<
              typename Kernel_traits<Point_3>::Kernel::FT>::type,
          class Has_filtered_predicates_tag = typename Kernel_traits<
              Point_3>::Kernel::Has_filtered_predicates_tag>
...
// utility class to select the right version of internal predicate
// Is_on_positive_side_of_plane_3
template <
    class Traits,
    class Is_floating_point = typename boost::is_floating_point<
        typename Kernel_traits<typename Traits::Point_3>::Kernel::FT>::type,
    class Has_filtered_predicates_tag = typename Kernel_traits<
        typename Traits::Point_3>::Kernel::Has_filtered_predicates_tag,
    class Has_cartesian_tag =
        typename Kernel_traits<typename Traits::Point_3>::Kernel::Kernel_tag,
    class Has_classical_point_type = typename boost::is_same<
...
template <class Traits, class Tag_use_advanced_filtering =
                            typename Use_advanced_filtering<Traits>::type>
...
public:
	...
	Is_on_positive_side_of_plane_3(const Traits& traits, const Point_3& p,
	                               const Point_3& q, const Point_3& r)
	    : plane(traits.construct_plane_3_object()(p, q, r)),
	      has_on_positive_side(traits.has_on_positive_side_3_object()) {}
...
// The main operator() first tries the static version of the predicate, then
// uses  interval arithmetic (the protector must be created before using this
// predicate)  and in case of failure, exact arithmetic is used.
template <class Kernel>
class Is_on_positive_side_of_plane_3<Convex_hull_traits_3<Kernel>, Tag_true> {
	typedef Simple_cartesian<CGAL::internal::Exact_field_selector<double>::Type>
	    PK;
	...
	// this function is a made from the statically filtered version of
	// Orientation_3
...
public:
	...
	Is_on_positive_side_of_plane_3(const Traits&, const Point_3& p_,
	                               const Point_3& q_, const Point_3& r_)
	...
	bool operator()(const Point_3& s) const {
		...
		if (static_res != STATIC_FILTER_FAILURE) return static_res == 1;

		try {
			if (ck_plane == NULL)
				ck_plane =
				    new typename CK::Plane_3(to_CK(p), to_CK(q), to_CK(r));
			return ck_plane->has_on_positive_side(to_CK(s));
		} catch (Uncertain_conversion_exception) {
			if (pk_plane == NULL)
				pk_plane =
				    new typename PK::Plane_3(to_PK(p), to_PK(q), to_PK(r));
...
BOOST_MPL_HAS_XXX_TRAIT_NAMED_DEF(Traits_has_typedef_Traits_xy_3, Traits_xy_3,
                                  false)
BOOST_MPL_HAS_XXX_TRAIT_NAMED_DEF(Traits_has_typedef_Traits_yz_3, Traits_xy_3,
                                  false)
BOOST_MPL_HAS_XXX_TRAIT_NAMED_DEF(Traits_has_typedef_Traits_xz_3, Traits_xy_3,
                                  false)

template <class T, bool has_projection_traits = Traits_has_typedef_Traits_xy_3<
                       T>::value&& Traits_has_typedef_Traits_yz_3<T>::value&&
...
void coplanar_3_hull(InputIterator first, InputIterator beyond,
	...
	typename Traits_xy_3::Left_turn_2 left_turn_in_xy =
	    traits_xy.left_turn_2_object();
	if (left_turn_in_xy(p1, p2, p3) || left_turn_in_xy(p2, p1, p3))
		...
		typename Traits_yz_3::Left_turn_2 left_turn_in_yz =
		    traits_yz.left_turn_2_object();
		if (left_turn_in_yz(p1, p2, p3) || left_turn_in_yz(p2, p1, p3))
			convex_hull_points_2(first, beyond, std::back_inserter(CH_2),
			                     traits_yz);
		else {
			Traits_xz_3 traits_xz;
			typename Traits_xz_3::Left_turn_2 left_turn_in_xz =
			    traits_xz.left_turn_2_object();
			CGAL_assertion(left_turn_in_xz(p1, p2, p3) ||
			               left_turn_in_xz(p2, p1, p3));
			convex_hull_points_2(first, beyond, std::back_inserter(CH_2),
			                     traits_xz);
		}
	}
...
void find_visible_set(
    TDS_2& tds, const typename Traits::Point_3& point,
	...
	start->vertex(0)->info() = start->vertex(1)->info() =
	    start->vertex(2)->info() = VISITED;
	...
	for (vis_it = visible.begin(); vis_it != visible.end(); vis_it++) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (f->info() == 0) {
				f->info() = VISITED;
				Plane_3 plane(f->vertex(0)->point(), f->vertex(1)->point(),
				              f->vertex(2)->point());
				...
				if (has_on_positive_side(plane, point)) {  // is visible
					...
					outside.insert(
					    std::make_pair(f->vertex(TDS_2::cw(ind)),
...
typename std::list<Point>::iterator farthest_outside_point(
    Face_handle f, std::list<Point>& outside_set, const Traits& traits) {
	typedef typename std::list<Point>::iterator Outside_set_iterator;
	CGAL_ch_assertion(!outside_set.empty());

	typename Traits::Plane_3 plane(f->vertex(0)->point(), f->vertex(1)->point(),
	                               f->vertex(2)->point());
...
template <class Face_handle, class Traits, class Point>
void partition_outside_sets(const std::list<Face_handle>& new_facets,
	...
	for (f_list_it = new_facets.begin();
	     (f_list_it != new_facets.end()) && (!vis_outside_set.empty());
	     ++f_list_it) {
		Face_handle f = *f_list_it;
		Is_on_positive_side_of_plane_3<Traits> is_on_positive_side(
		    traits, f->vertex(0)->point(), f->vertex(1)->point(),
		    f->vertex(2)->point());
...
template <class TDS_2, class Traits>
void ch_quickhull_3_scan(TDS_2& tds,
	...
	typedef std::map<typename TDS_2::Vertex_handle, typename TDS_2::Edge>
	    Border_edges;
	...
	while (!pending_facets.empty()) {
		...
		Outside_set_iterator farthest_pt_it =
		    farthest_outside_point(f_handle, f_handle->points, traits);
		...
		for (vis_set_it = visible_set.begin(); vis_set_it != visible_set.end();
			...
			if (!point_list.empty()) {
				vis_outside_set.splice(vis_outside_set.end(), point_list,
		...
		// If we want to reuse the faces we must only pass |edges| many, and
		// call delete_face for the others. Also create facets if necessary
		...
		Vertex_handle vh = tds.star_hole(
		    edges.begin(), edges.end(), visible_set.begin(), visible_set.end());
		vh->point() = farthest_pt;
		vh->info() = 0;

		// now partition the set of outside set points among the new facets.

		partition_outside_sets(visible_set, vis_outside_set, pending_facets,
		                       traits);
	}
}
...
void non_coplanar_quickhull_3(std::list<typename Traits::Point_3>& points,
	...
	for (Face_iterator fit = tds.faces_begin(); fit != tds.faces_end(); ++fit) {
		Is_on_positive_side_of_plane_3<Traits> is_on_positive_side(
		    traits, fit->vertex(0)->point(), fit->vertex(1)->point(),
		    fit->vertex(2)->point());
...
class Build_convex_hull_from_TDS_2 : public CGAL::Modifier_base<HDS> {
	...
	static unsigned get_vertex_index(Vertex_map& vertex_map,
	                                 ...
	                                 Builder& builder, unsigned& vindex) {
		std::pair<typename Vertex_map::iterator, bool> res =
		    vertex_map.insert(std::make_pair(vh, vindex));
		if (res.second) {
...
public:
	...
	void operator()(HDS& hds) {
		...
		unsigned vindex = 0;
		for (typename TDS::Face_iterator it = t.faces_begin();
		     it != t.faces_end(); ++it) {
			unsigned i0 =
			    get_vertex_index(vertex_map, it->vertex(0), B, vindex);
			unsigned i1 =
			    get_vertex_index(vertex_map, it->vertex(1), B, vindex);
			unsigned i2 =
			    get_vertex_index(vertex_map, it->vertex(2), B, vindex);
			B.begin_facet();
...
void ch_quickhull_polyhedron_3(std::list<typename Traits::Point_3>& points,
	...
	typedef Triangulation_data_structure_2<
	    ...
	    Convex_hull_face_base_2<int, Traits>>
	    Tds;
	...
	if (coplanar(*point1_it, *point2_it, *point3_it, *max_it)) {
		coplanar_3_hull(points.begin(), points.end(), *point1_it, *point2_it,
		                *point3_it, P, traits);
	} else {
		Tds tds;
		Vertex_handle v0 = tds.create_vertex();
		v0->set_point(*point1_it);
		Vertex_handle v1 = tds.create_vertex();
		v1->set_point(*point2_it);
		Vertex_handle v2 = tds.create_vertex();
		v2->set_point(*point3_it);
		...
		if (!points.empty()) {
			...
			internal::Build_convex_hull_from_TDS_2<
			    typename Polyhedron_3::HalfedgeDS, Tds>
			    builder(tds);
			P.delegate(builder);
		} else
			P.make_tetrahedron(v0->point(), v1->point(), v2->point(),
			                   v3->point());
	}
}
...
void convex_hull_3(InputIterator first, InputIterator beyond, Object& ch_object,
	...
	// If the first and last point are equal the collinearity test some lines
	...
	} else if (size == 3)  // 3 points
	{
		...
		Triangle_3 tri = construct_triangle(
		    *(points.begin()), *(++points.begin()), *(--points.end()));
	...
	typename internal::Convex_hull_3::Default_polyhedron_for_Chull_3<
	    Traits>::type P;
	...
	if (!collinear(*minx, *maxx, *miny)) {
		internal::Convex_hull_3::ch_quickhull_polyhedron_3(points, minx, maxx,
		                                                   miny, P, traits);
	} else {
		internal::Convex_hull_3::ch_quickhull_polyhedron_3(
		    points, point1_it, point2_it, point3_it, P, traits);
	}
	...
	if (boost::next(P.vertices_begin(), 3) == P.vertices_end()) {
		...
		typename Traits::Construct_triangle_3 construct_triangle =
		    traits.construct_triangle_3_object();
		Triangle_3 tri = construct_triangle(
		    P.halfedges_begin()->vertex()->point(),
...
void convex_hull_3(InputIterator first, InputIterator beyond,
	...
	typedef typename internal::Convex_hull_3::Default_traits_for_Chull_3<
	    Point_3>::type Traits;
...
void convex_hull_3(InputIterator first, InputIterator beyond,
	...
	CGAL_ch_precondition_msg(
	    point2_it != points.end(),
	...
	CGAL_ch_precondition_msg(point3_it != points.end(),
	                         "Only two points with different coordinates; "
	                         "cannot construct polyhedron.");

	while (point3_it != points.end() &&
	       collinear(*point1_it, *point2_it, *point3_it))
		++point3_it;

	CGAL_ch_precondition_msg(
	    point3_it != points.end(),
	    "All points are collinear; cannot construct polyhedron.");

	polyhedron.clear();
	// result will be a polyhedron
	internal::Convex_hull_3::ch_quickhull_polyhedron_3(
	    points, point1_it, point2_it, point3_it, polyhedron, traits);
}
