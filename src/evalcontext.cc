...
/*!
  Resolves arguments specified by evalctx, using args to lookup positional
  arguments. Returns an AssignmentMap (string -> Expression*)
*/
...
#ifdef DEBUG
std::string EvalContext::dump(const AbstractModule *mod,
                              const ModuleInstantiation *inst) {
	std::stringstream s;
	if (inst)
		s << boost::format("EvalContext %p (%p) for %s inst (%p)") % this %
	...
	for (size_t i = 0; i < this->eval_arguments.size(); i++) {
		s << boost::format("    %s = %s") % this->eval_arguments[i].name %
		         this->eval_arguments[i].expr;
	}
