...
/* Given a 3D PolySet with near planar polygonal faces, tessellate the
   ...
   input polyset has simple polygon faces with no holes. The tessellation will
   be robust wrt. degenerate and self-intersecting
*/
void tessellate_faces(const PolySet &inps, PolySet &outps) {
    ...
    for (const auto &faces : polygons) {
        ...
        if (faces[0].size() == 3) {
            ...
            err = GeometryUtils::tessellatePolygonWithHoles(verts, faces,
                                                            triangles, nullptr);
        }
        if (!err) {
