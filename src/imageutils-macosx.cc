...
CGDataConsumerRef CGDataConsumerCreateWithOstream(std::ostream &output) {
	...
	CGDataConsumerRef dc =
	    CGDataConsumerCreate((void *)(&output), &dc_callbacks);
	return dc;
}
...
bool write_png(std::ostream &output, unsigned char *pixels, int width,
	...
	kCFStringEncodingUTF8); CFURLRef fileURL =
	CFURLCreateWithFileSystemPath(kCFAllocatorDefault, fname,
	kCFURLPOSIXPathStyle, false); if (!fileURL) { std::cerr << "Unable to create
	file URL ref."; return false;
	}
