...
//
// $URL:
// svn+ssh://scm.gforge.inria.fr/svn/cgal/trunk/Nef_3/include/CGAL/Nef_3/OGL_helper.h
...
public:
    Double_triple() { coords_[0] = coords_[1] = coords_[2] = 0.0; }
...
static std::ostream& operator<<(std::ostream& os, const Double_triple& t) {
    os << "(" << t[0] << "," << t[1] << "," << t[2] << ")";
    return os;
}
...
public:
    ...
    void set_normal(double x, double y, double z, bool m) {
        ...
        normal_ = Double_triple(x / l, y / l, z / l);
        mark_ = m;
    }
    ...
    double* normal() const { return static_cast<double*>(normal_); }

    void new_facet_cycle() { fc_ends_.push_back(coords_.size()); }

    unsigned number_of_facet_cycles() const { return fc_ends_.size(); }
    ...
    Coord_iterator facet_cycle_begin(unsigned i) {
        ...
        if (i == 0)
            ...
            return coords_.begin() + fc_ends_[i];
    }

    Coord_iterator facet_cycle_end(unsigned i) {
        CGAL_assertion(i < number_of_facet_cycles());
        if (i < fc_ends_.size() - 1)
            return coords_.begin() + fc_ends_[i + 1];
        else
            return coords_.end();
    }
...
inline void CGAL_GLU_TESS_CALLBACK endCallback(void) { glEnd(); }
...
inline void CGAL_GLU_TESS_CALLBACK vertexCallback(GLvoid* vertex,
    ...
    //    CGAL_NEF_TRACEN("vertexCallback coord
    //    "<<pc[0]<<","<<pc[1]<<","<<pc[2]); CGAL_NEF_TRACEN("vertexCallback
    //    normal "<<pu[0]<<","<<pu[1]<<","<<pu[2]);
    glNormal3dv(pu);
    glVertex3dv(pc);
}

inline void CGAL_GLU_TESS_CALLBACK combineCallback(GLdouble coords[3],
                                                   GLvoid * [4], GLfloat[4],
                                                   GLvoid** dataOut) {
    ...
    if (dataOut) {
        ...
        *dataOut = n;
        ...
        for (std::list<GLdouble*>::const_iterator i = pcache.begin();
             i != pcache.end(); i++)
            delete[] * i;
        pcache.clear();
    }
...
public:
    ...
    */
    ...
    void push_back(const DFacet& f) { halffacets_.push_back(f); }

    void toggle(int index) { switches[index] = !switches[index]; }

    void set_style(int index) { style = index; }
    ...
    // Overridden in CGAL_renderer
    virtual CGAL::Color getFacetColor(Halffacet_iterator /*f*/,
                                      bool /*is_back_facing*/) const {
    ...
    void draw(Halffacet_iterator f, bool is_back_facing) const {
        ...
        gluTessCallback(tess_, GLenum(GLU_TESS_VERTEX_DATA),
                        (GLvoid(CGAL_GLU_TESS_CALLBACK*)(CGAL_GLU_TESS_DOTS)) &
                            vertexCallback);
        ...
        for (unsigned i = 0; i < f->number_of_facet_cycles(); ++i) {
            ...
            // put all vertices in facet cycle into contour:
            for (cit = f->facet_cycle_begin(i); cit != f->facet_cycle_end(i);
                 ++cit) {
    ...
    void fill_display_lists() {
        ...
        Vertex_iterator v;
        for (v = vertices_.begin(); v != vertices_.end(); ++v) draw(v);
        glEndList();

        glNewList(object_list_ + 1, GL_COMPILE);
        Edge_iterator e;
        for (e = edges_.begin(); e != edges_.end(); ++e) draw(e);
        glEndList();
    ...
    void debug(std::ostream& os = std::cerr) const {
        ...
        os << "Facets:" << std::endl;
        Halffacet_iterator f;
        for (f = halffacets_.begin(); f != halffacets_.end(); ++f) f->debug();
        os << std::endl;
        os << std::endl;
    }
...
public:
    ...
    typedef
        typename SNC_structure::Halfedge_const_iterator Halfedge_const_iterator;
    ...
    typedef
        typename SNC_structure::SHalfedge_const_handle SHalfedge_const_handle;
    typedef
        typename SNC_structure::SHalfloop_const_handle SHalfloop_const_handle;

    typedef typename SNC_structure::Vertex_const_handle Vertex_const_handle;
    typedef typename SNC_structure::Halfedge_const_handle Halfedge_const_handle;
    typedef
        typename SNC_structure::Halffacet_const_handle Halffacet_const_handle;
...
private:
    static OGL::Double_point double_point(const Point_3& p) {
        return OGL::Double_point(CGAL::to_double(p.x()), CGAL::to_double(p.y()),
                                 CGAL::to_double(p.z()));
    }
    ...
    static bool same_orientation(Plane_3 p1, Plane_3 p2) {
        if (p1.a() != 0) return CGAL::sign(p1.a()) == CGAL::sign(p2.a());
        if (p1.b() != 0) return CGAL::sign(p1.b()) == CGAL::sign(p2.b());
    ...
    static void draw(Halffacet_const_handle f, const Nef_polyhedron&,
        ...
        if (f->incident_volume()->mark())
            return;  // Skip halffaces facing solid volume
        OGL::DFacet g;
        Halffacet_cycle_const_iterator fc;  // all facet cycles:
        CGAL_forall_facet_cycles_of(
            fc, f) if (fc.is_shalfedge()) {  // non-trivial facet cycle
            g.new_facet_cycle();
            ...
            CGAL_For_all(hc, he) {  // all vertex coordinates in facet cycle
                ...
                //	      CGAL_NEF_TRACEN(" ");CGAL_NEF_TRACEN("facet" <<
                //sp);
                g.push_back_vertex(double_point(sp));
            }
        }
        Vector_3 v = f->plane().orthogonal_vector();
        g.set_normal(CGAL::to_double(v.x()), CGAL::to_double(v.y()),
                     CGAL::to_double(v.z()), f->mark());
        P.push_back(g);
    }
    ...
    static void set_R(Bbox_3& bbox, const Nef_polyhedron& N) {
        ...
        Vertex_const_iterator vi;
        CGAL_forall_vertices(vi, N) if (N.is_standard(vi)) return;
        bbox = Bbox_3(bbox.xmin() * 10, bbox.ymin() * 10, bbox.zmin() * 10,
                      bbox.xmax() * 10, bbox.ymax() * 10, bbox.zmax() * 10);
    }

public:
    static void convert_to_OGLPolyhedron(const Nef_polyhedron& N,
                                         CGAL::OGL::Polyhedron* P) {
