...
ValuePtr builtin_rands(const Context *, const EvalContext *evalctx)
{
	...
	if (n == 3 || n == 4) {
		...
		if (min == max) { // Boost doesn't allow min == max
			for (size_t i = 0; i < numresults; i++) vec.push_back(ValuePtr(min));
		} else {
	...
	for (size_t i = 0; i < findThisSize; i++) {
		...
		for (size_t j = 0; j < searchTableSize; j++) {
			...
			if (entryVec.size() <= index_col_num) {
				PRINTB("WARNING: Invalid entry in search vector at index %d, required number of values in "
							 "the entry: %d. Invalid entry: %s",
							 j % (index_col_num + 1) % table[j]);
				return Value::VectorType();
			}
			const gchar *ptr_st =
					g_utf8_offset_to_pointer(entryVec[index_col_num]->toString().c_str(), 0);
			if (ptr_ft && ptr_st && (g_utf8_get_char(ptr_ft) == g_utf8_get_char(ptr_st))) {
				matchCount++;
				if (num_returns_per_match == 1) {
					returnvec.push_back(ValuePtr(double(j)));
...
ValuePtr builtin_search(const Context *, const EvalContext *evalctx)
{
	...
	if (findThis->type() == Value::ValueType::NUMBER) {
		...
		if (searchTable->type() == Value::ValueType::STRING) {
			...
			returnvec = search(findThis->toString(), searchTable->toVector(), num_returns_per_match,
												 index_col_num);
		}
...
{
	ValuePtr val =
			(evalctx->numArgs() == 0) ? builtin_version(ctx, evalctx) : evalctx->getArgValue(0);
	double y, m, d = 0;
