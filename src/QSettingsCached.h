...
public:
	...
	inline void setValue(const QString &key, const QVariant &value)
	{
		...
		// Disabling forced sync to persisted storage on write. Will rely on automatic behavior of
		// QSettings qsettingsPointer->sync(); // force write to file system on each modification of
		// open scad settings
	}
	...
	inline void remove(const QString &key)
	{
		...
		// Disabling forced sync to persisted storage on write. Will rely on automatic behavior of
		// QSettings qsettingsPointer->sync();
	}

	inline bool contains(const QString &key) { return qsettingsPointer->contains(key); }
