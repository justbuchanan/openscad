...
namespace boosty {
	...
	inline fs::path canonical(fs::path p, fs::path p2) { return fs::canonical(p, p2); }

	inline fs::path canonical(fs::path p) { return fs::canonical(p); }
...
#error you should be using a newer version of boost on win/mac
	...
	inline fs::path canonical(fs::path p) { return canonical(p, fs::current_path()); }
