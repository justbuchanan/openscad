...
ClipperLib::Path fromOutline2d(const Outline2d &outline,
    ...
    if (!keep_orientation && !ClipperLib::Orientation(p))
...
ClipperLib::Paths fromPolygon2d(const Polygon2d &poly) {
    ...
    for (const auto &outline : poly.outlines()) {
        result.push_back(
            fromOutline2d(outline, poly.isSanitized() ? true : false));
    }
    return result;
}
...
/*!
        ...
        We could use a Paths structure, but we'd have to check the orientation
   of each path before adding it to the Polygon2d.
*/
    ...
    while (node) {
        ...
        if (cleaned_path.size() >= 3) {
            for (const auto &ip : cleaned_path) {
                outline.vertices.emplace_back(1.0 * ip.X / CLIPPER_SCALE,
...
Polygon2d *apply(const std::vector<ClipperLib::Paths> &pathsvector,
    ...
    if (clipType == ClipperLib::ctIntersection && pathsvector.size() >= 2) {
        ...
        for (unsigned int i = 1; i < pathsvector.size(); i++) {
            ...
            clipper.Execute(clipType, result, ClipperLib::pftNonZero,
    ...
    for (const auto &paths : pathsvector) {
        clipper.AddPaths(
            paths, first ? ClipperLib::ptSubject : ClipperLib::ptClip, true);
...
// This is a copy-paste from ClipperLib with the modification that the union
...
static void minkowski_outline(const ClipperLib::Path &poly,
                              ...
                              ClipperLib::Paths &quads, bool isSum,
                              bool isClosed) {
    ...
    if (isSum)
        for (size_t i = 0; i < pathCnt; ++i) {
            ...
            for (size_t j = 0; j < poly.size(); ++j)
                p.push_back(ClipperLib::IntPoint(path[i].X + poly[j].X,
                                                 path[i].Y + poly[j].Y));
            pp.push_back(p);
        }
    else
...
Polygon2d *applyMinkowski(const std::vector<const Polygon2d *> &polygons) {
    ...
    for (size_t i = 1; i < polygons.size(); i++) {
        ...
        // This union operation must be performed at each interation since the
        // minkowski_terms now contain lots of small quads
        c.Clear();
        c.AddPaths(minkowski_terms, ClipperLib::ptSubject, true);

        if (i != polygons.size() - 1) {
            c.Execute(ClipperLib::ctUnion, lhs, ClipperLib::pftNonZero,
                      ClipperLib::pftNonZero);
        }
    }

    ClipperLib::PolyTree polytree;
    c.Execute(ClipperLib::ctUnion, polytree, ClipperLib::pftNonZero,
              ClipperLib::pftNonZero);

    return toPolygon2d(polytree);
}

Polygon2d *applyOffset(const Polygon2d &poly, double offset,
                       ClipperLib::JoinType joinType, double miter_limit,
