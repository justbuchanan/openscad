...
namespace ClipperUtils {
	...
	*/
	...
	ClipperLib::Paths process(const ClipperLib::Paths &polygons, ClipperLib::ClipType cliptype,
	...
	/*!
		...
		May return an empty Polygon2d, but will not return nullptr.
	 */
	Polygon2d *apply(const std::vector<const Polygon2d *> &polygons, ClipperLib::ClipType clipType)
	{
	...
	// This is a copy-paste from ClipperLib with the modification that the union operation is not
	// performed The reason is numeric robustness. With the insides missing, the intersection points
	// created by the union operation may (due to rounding) be located at slightly different locations
	// than the original geometry and this can give rise to cracks
	...
	// the minkowski sum.
	static void fill_minkowski_insides(const ClipperLib::Paths &a, const ClipperLib::Paths &b,
