...
void Renderer::setColor(ColorMode colormode, const float color[4], GLint *shaderinfo) const
{
	...
	if (getColor(colormode, basecol)) {
		if (colormode == ColorMode::BACKGROUND) {
			basecol = {color[0] >= 0 ? color[0] : basecol[0], color[1] >= 0 ? color[1] : basecol[1],
...
void Renderer::render_surface(shared_ptr<const Geometry> geom, csgmode_e csgmode,
															const Transform3d &m, GLint *shaderinfo)
{
