...
void Renderer::setColor(const float color[4], GLint *shaderinfo) const {
	...
	if (shaderinfo) {
		...
		glUniform4f(shaderinfo[2], (c[0] + 1) / 2, (c[1] + 1) / 2,
		            (c[2] + 1) / 2, 1.0);
	}
#endif
}

void Renderer::setColor(ColorMode colormode, const float color[4],
...
void Renderer::render_edges(shared_ptr<const Geometry> geom,
                            csgmode_e csgmode) {
	auto ps = dynamic_pointer_cast<const PolySet>(geom);
