...
public:
    ...
    public:
        ...
        CGAL::Mesh_2::Face_badness operator()(const typename T::Face_handle &,
                                              Quality &q) const {
            q = 1;
            return CGAL::Mesh_2::NOT_BAD;
        }
...
/* Tessellation of 3d PolySet faces
         ...
         (in reality as of writing, we dont need to do a back-projection from
   2d->3d because the algorithm we are using doesn't create any new points, and
   we can just use a 'map' to associate 3d points with 2d points).

         The code assumes the input polygons are simple, non-intersecting,
   without holes, without duplicate input points, and with proper orientation.

         The purpose of this code is originally to fix github issue 349. Our
   CGAL kernel does not accept polygons for Nef_Polyhedron_3 if each of the
         points is not exactly coplanar. "Near-planar" or "Almost planar"
   polygons often occur due to rounding issues on, for example, polyhedron()
   input. By tessellating the 3d polygon into individual smaller tiles that are
   perfectly coplanar (triangles, for example), we can get CGAL to accept the
   polyhedron() input.
*/
...
/* triangulate the given 3d polygon using CGAL's 2d Constrained Delaunay
         algorithm. Project the polygon's points into 2d using the given
   projection before performing the triangulation. This code assumes input
   polygon is simple, no holes, no self-intersections, no duplicate points, and
   is properly oriented. output is a sequence of 3d triangles. */
bool triangulate_polygon(const PolySet::Polygon &pgon,
    ...
    try {
        ...
        CGAL::refine_Delaunay_mesh_2_without_edge_refinement(
            cdt, list_of_seeds.begin(), list_of_seeds.end(),
            DummyCriteria<CDT>());
        ...
        // Using failure exception to catch precondition errors for malformed
        // polygons in e.g. CGAL::orientation_2().
...
/* Given a 3d PolySet with 'near planar' polygonal faces, Tessellate the
         ...
         using CGAL's Constrained Delaunay algorithm. This code assumes the
   input polyset has simple polygon faces with no holes, no self intersections,
   no duplicate points, and proper orientation. */
void tessellate_faces(const PolySet &inps, PolySet &outps) {
    ...
    if (degeneratePolygons > 0)
        PRINT("WARNING: PolySet has degenerate polygons");
}
}  // namespace PolysetUtils
