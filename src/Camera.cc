...
void Camera::gimbalDefaultTranslate() {  // match the GUI viewport numbers
                                         // (historical reasons)
    object_trans.x() *= -1;
    object_trans.y() *= -1;
    object_trans.z() *= -1;
...
void Camera::viewAll(const BoundingBox &bbox) {
    ...
    PRINTDB("modified center x y z %f %f %f",
...
double Camera::zoomValue() { return viewer_distance; }
...
std::string Camera::statusText() {
    ...
    fmt % object_trans.x() % object_trans.y() % object_trans.z() %
        object_rot.x() % object_rot.y() % object_rot.z() % viewer_distance;
    return fmt.str();
}
