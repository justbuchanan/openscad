...
{
	boost::format fmt(
			_("Viewport: translate = [ %.2f %.2f %.2f ], rotate = [ %.2f %.2f %.2f ], distance = %.2f"));
	fmt % object_trans.x() % object_trans.y() % object_trans.z() % object_rot.x() % object_rot.y() %
			object_rot.z() % viewer_distance;
	return fmt.str();
}
