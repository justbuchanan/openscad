...
#define PRINT_DEPRECATION(_fmt, _arg)                      \
    do {                                                   \
        printDeprecation(str(boost::format(_fmt) % _arg)); \
    } while (0)

/* PRINT statements come out in same window as ECHO.
 usage: PRINTB("Var1: %s Var2: %i", var1 % var2 ); */
void PRINT(const std::string &msg);
#define PRINTB(_fmt, _arg)                      \
    do {                                        \
        PRINT(str(boost::format(_fmt) % _arg)); \
    } while (0)

void PRINT_NOCACHE(const std::string &msg);
#define PRINTB_NOCACHE(_fmt, _arg)                      \
    do {                                                \
        PRINT_NOCACHE(str(boost::format(_fmt) % _arg)); \
    } while (0)

void PRINT_CONTEXT(const class Context *ctx, const class Module *mod,
                   const class ModuleInstantiation *inst);
...
#define PRINTDB(_fmt, _arg)                                         \
    do {                                                            \
        try {                                                       \
            PRINTDEBUG(std::string(__FILE__),                       \
                       str(boost::format(_fmt) % _arg));            \
