...
/*! \class DxfData
        ...
        The DxfData class fulfils multiple tasks, partially for historical
  reasons.
  ...
  operations. Used for tesselation into triangles 4) Store 2D polygons before
  exporting to DXF
...
#define ADD_LINE(_x1, _y1, _x2, _y2)                                           \
    ...
    //
    // Parse DXF file. Will populate this->points, this->dims, lines and
    // blockdata
    //
    while (!stream.eof()) {
        ...
        try {
            if (id >= 10 && id <= 16) {
                if (in_blocks_section) {
                    ...
                    coords[id - 10][0] =
                        boost::lexical_cast<double>(data) * scale;
                } else {
                    coords[id - 10][0] =
            ...
            switch (id) {
                case 0:
                    if (mode == "SECTION") {
                        ...
                        for (int i = 0; i < n; i++) {
                            ...
                            ADD_LINE(cos(a1) * radius + center[0],
                        ...
                        n = static_cast<int>(
                            ceil(n * (arc_stop_angle - arc_start_angle) / 360));
                        for (int i = 0; i < n; i++) {
                            double a1 =
                                ((arc_stop_angle - arc_start_angle) * i) / n;
                            double a2 =
                                ((arc_stop_angle - arc_start_angle) * (i + 1)) /
                                n;
                        ...
                        // Commented code is meant as documentation of vector
                        // math
                        while (ellipse_start_angle > ellipse_stop_angle)
                            ellipse_stop_angle += 2 * M_PI;
                        //				Vector2d center(xverts[0],
                        //yverts[0]);
                        Vector2d center(xverts.at(0), yverts.at(0));
                        //				Vector2d ce(xverts[1],
                        //yverts[1]);
                        ...
                        double rot_angle;
                        {
                            //					double dot = ce.dot(Vector2d(1.0,
                            //0.0));
                            double dot = ce[0];
                        ...
                        // the ratio stored in 'radius; due to the parser code
                        ...
                        n = static_cast<int>(
                            ceil(n * sweep_angle / (2 * M_PI)));
                        //				Vector2d p1;
                        Vector2d p1{0.0, 0.0};
                        for (int i = 0; i <= n; i++) {
                            double a =
                                (ellipse_start_angle + sweep_angle * i / n);
                            //					Vector2d p2(cos(a)*r_major,
                            //sin(a)*r_minor);
                            Vector2d p2(cos(a) * r_major, sin(a) * r_minor);
                            //					p2.rotate(rot_angle);
                            Vector2d p2_rot(
                                cos(rot_angle) * p2[0] - sin(rot_angle) * p2[1],
                                sin(rot_angle) * p2[0] +
                                    cos(rot_angle) * p2[1]);
                            //					p2 += center;
                            p2_rot[0] += center[0];
                        ...
                        // scale is stored in ellipse_start|stop_angle, rotation
                        // in arc_start_angle; due to the parser code not
                        // checking entity type
                        int n = blockdata[iddata].size();
                        for (int i = 0; i < n; i++) {
                            double a = arc_start_angle * M_PI / 180.0;
                            double lx1 =
                                this->points[blockdata[iddata][i].idx[0]][0] *
                                ellipse_start_angle;
                            double ly1 =
                                this->points[blockdata[iddata][i].idx[0]][1] *
                                ellipse_stop_angle;
                            double lx2 =
                                this->points[blockdata[iddata][i].idx[1]][0] *
                                ellipse_start_angle;
                            double ly2 =
                                this->points[blockdata[iddata][i].idx[1]][1] *
                                ellipse_stop_angle;
                            double px1 = (cos(a) * lx1 - sin(a) * ly1) * scale +
                                         xverts.at(0);
                            double py1 = (sin(a) * lx1 + cos(a) * ly1) * scale +
                                         yverts.at(0);
                            double px2 = (cos(a) * lx2 - sin(a) * ly2) * scale +
                                         xverts.at(0);
                            double py2 = (sin(a) * lx2 + cos(a) * ly2) * scale +
                                         yverts.at(0);
                    ...
                    if (mode == "INSERT") {
                        ellipse_start_angle = ellipse_stop_angle =
                            1.0;  // scale
                    }
                    break;
                case 1:
                ...
                case 10:
                    if (in_blocks_section) {
                        ...
                        xverts.push_back(
                            (boost::lexical_cast<double>(data) - xorigin) *
                            scale);
                    }
                    break;
                case 11:
                ...
                case 20:
                    if (in_blocks_section) {
                        ...
                        yverts.push_back(
                            (boost::lexical_cast<double>(data) - yorigin) *
                            scale);
                    }
                    break;
                case 21:
            ...
            PRINTB("WARNING: not enough input values for %s in '%s'",
                   data % filename);
        }
    }
    ...
    for (const auto &i : unsupported_entities_list) {
        if (layername.empty()) {
            PRINTB("WARNING: Unsupported DXF Entity '%s' (%x) in %s.",
                   i.first % i.second %
                       QuotedString(
                           boostfs_uncomplete(filename, fs::current_path())
                               .generic_string()));
        } else {
            PRINTB(
                "WARNING: Unsupported DXF Entity '%s' (%x) in layer '%s' of "
                "%s.",
...
void DxfData::fixup_path_direction() {
    for (size_t i = 0; i < this->paths.size(); i++) {
        ...
        double ax = this->points[this->paths[i].indices[a]][0] -
                    this->points[this->paths[i].indices[b]][0];
        double ay = this->points[this->paths[i].indices[a]][1] -
                    this->points[this->paths[i].indices[b]][1];
        double cx = this->points[this->paths[i].indices[c]][0] -
                    this->points[this->paths[i].indices[b]][0];
        double cy = this->points[this->paths[i].indices[c]][1] -
                    this->points[this->paths[i].indices[b]][1];
#if 0
		printf("Rotate check:\n");
...
#endif
        // FIXME: atan2() usually takes y,x. This variant probably makes the
        // path clockwise..
        if (atan2(ax, ay) < atan2(cx, cy)) {
            std::reverse(this->paths[i].indices.begin(),
                         this->paths[i].indices.end());
        }
    }
}
...
Polygon2d *DxfData::toPolygon2d() const {
    ...
    for (size_t i = 0; i < this->paths.size(); i++) {
        ...
        // We don't support open paths; closing them to be compatible with
        // existing behavior
