...
namespace Settings {
	...
	SettingsEntry::SettingsEntry(const std::string category, const std::string name,
															 const Value range, const Value def)
	...
	const std::string &SettingsEntry::category() const { return _category; }

	const std::string &SettingsEntry::name() const { return _name; }

	const Value &SettingsEntry::defaultValue() const { return _default; }

	const Value &SettingsEntry::range() const { return _range; }

	bool SettingsEntry::is_default() const { return _value == _default; }
	...
	static Value values(std::string s1, std::string s1disp, std::string s2, std::string s2disp,
											std::string s3, std::string s3disp)
	{
		Value::VectorType v;
	...
	static Value values(std::string s1, std::string s1disp, std::string s2, std::string s2disp,
											std::string s3, std::string s3disp, std::string s4, std::string s4disp)
	{
		Value::VectorType v;
		v += ValuePtr(value(s1, s1disp)), ValuePtr(value(s2, s2disp)), ValuePtr(value(s3, s3disp)),
				ValuePtr(value(s4, s4disp));
		return v;
	}
	...
	const Value &Settings::defaultValue(const SettingsEntry &entry) { return entry._default; }

	const Value &Settings::get(const SettingsEntry &entry) { return entry._value; }

	void Settings::set(SettingsEntry &entry, const Value &val) { entry._value = val; }
	 ...
	 * can be translated.
	 */
	SettingsEntry Settings::showWarningsIn3dView("3dview", "showWarningsIn3dView", Value(true),
																							 Value(true));
	SettingsEntry Settings::indentationWidth("editor", "indentationWidth", Value(RangeType(1, 16)),
																					 Value(4));
	SettingsEntry Settings::tabWidth("editor", "tabWidth", Value(RangeType(1, 16)), Value(4));
	SettingsEntry Settings::lineWrap("editor", "lineWrap",
																	 values("None", _("None"), "Char",
																					_("Wrap at character boundaries"), "Word",
																					_("Wrap at word boundaries")),
																	 Value("Word"));
	SettingsEntry Settings::lineWrapIndentationStyle("editor", "lineWrapIndentationStyle",
																									 values("Fixed", _("Fixed"), "Same", _("Same"),
																													"Indented", _("Indented")),
																									 Value("Fixed"));
	SettingsEntry Settings::lineWrapIndentation("editor", "lineWrapIndentation",
																							Value(RangeType(0, 999)), Value(4));
	SettingsEntry Settings::lineWrapVisualizationBegin("editor", "lineWrapVisualizationBegin",
																										 values("None", _("None"), "Text", _("Text"),
																														"Border", _("Border"), "Margin",
																														_("Margin")),
																										 Value("None"));
	SettingsEntry Settings::lineWrapVisualizationEnd("editor", "lineWrapVisualizationEnd",
																									 values("None", _("None"), "Text", _("Text"),
																													"Border", _("Border"), "Margin",
																													_("Margin")),
																									 Value("Border"));
	SettingsEntry Settings::showWhitespace("editor", "showWhitespaces",
																				 values("Never", _("Never"), "Always", _("Always"),
																								"AfterIndentation", _("After indentation")),
																				 Value("Never"));
	SettingsEntry Settings::showWhitespaceSize("editor", "showWhitespacesSize",
																						 Value(RangeType(1, 16)), Value(2));
	SettingsEntry Settings::autoIndent("editor", "autoIndent", Value(true), Value(true));
	SettingsEntry Settings::backspaceUnindents("editor", "backspaceUnindents", Value(true),
																						 Value(false));
	SettingsEntry Settings::indentStyle("editor", "indentStyle",
																			values("Spaces", _("Spaces"), "Tabs", _("Tabs")),
																			Value("Spaces"));
	SettingsEntry Settings::tabKeyFunction("editor", "tabKeyFunction",
																				 values("Indent", _("Indent"), "InsertTab",
																								_("Insert Tab")),
																				 Value("Indent"));
	SettingsEntry Settings::highlightCurrentLine("editor", "highlightCurrentLine", Value(true),
																							 Value(true));
	SettingsEntry Settings::enableBraceMatching("editor", "enableBraceMatching", Value(true),
																							Value(true));
	SettingsEntry Settings::enableLineNumbers("editor", "enableLineNumbers", Value(true),
																						Value(true));
	...
	SettingsEntry Settings::inputTranslationXVPRel("input", "translationXVPRel", axisValues(),
																								 Value(""));
	SettingsEntry Settings::inputTranslationYVPRel("input", "translationYVPRel", axisValues(),
																								 Value(""));
	SettingsEntry Settings::inputTranslationZVPRel("input", "translationZVPRel", axisValues(),
																								 Value(""));
	...
	SettingsEntry Settings::inputButton1("input", "button1", buttonValues(),
																			 Value("viewActionResetView"));
	...
	SettingsEntry Settings::axisTrim0("input", "axisTrim0", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim1("input", "axisTrim1", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim2("input", "axisTrim2", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim3("input", "axisTrim3", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim4("input", "axisTrim4", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim5("input", "axisTrim5", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim6("input", "axisTrim6", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim7("input", "axisTrim7", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim8("input", "axisTrim8", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisTrim9("input", "axisTrim9", RangeType(-1.0, 1.0), Value("0.00"));
	SettingsEntry Settings::axisDeadzone0("input", "axisDeadzone0", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone1("input", "axisDeadzone1", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone2("input", "axisDeadzone2", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone3("input", "axisDeadzone3", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone4("input", "axisDeadzone4", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone5("input", "axisDeadzone5", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone6("input", "axisDeadzone6", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone7("input", "axisDeadzone7", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone8("input", "axisDeadzone8", RangeType(0.0, 1.0),
																				Value("0.10"));
	SettingsEntry Settings::axisDeadzone9("input", "axisDeadzone9", RangeType(0.0, 1.0),
																				Value("0.10"));
} // namespace Settings
