...
/*!
        FIXME: Implement an LRU scheme to avoid having an ever-growing module
   cache
*/
...
/*!
        Reevaluate the given file and all it's dependencies and recompile
   anything needing reevaluation. Updates the cache if necessary. The given
   filename must be absolute.

        Sets the module reference to the new module, or nullptr on any error
   (e.g. compile error or file not found).

        Returns the latest mod time of the modul or its dependencies or
   includes.
*/
time_t ModuleCache::evaluate(const std::string &filename, FileModule *&module) {
    ...
    std::string cache_id =
        str(boost::format("%x.%x") % st.st_mtime % st.st_size);
    ...
    if (found) {
        ...
        // if (!shouldCompile) PRINTB("Using cached library: %s (%p)", filename
        // % lib_mod);
#endif
    ...
    if (shouldCompile) {
        ...
        lib_mod =
            parse(entry.parsed_module, textbuf.str().c_str(), pathname, false)
                ? entry.parsed_module
                : nullptr;
...
void ModuleCache::clear() { this->entries.clear(); }
