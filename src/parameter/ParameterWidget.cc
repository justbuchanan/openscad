...
ParameterWidget::ParameterWidget(QWidget *parent) : QWidget(parent) {
    ...
    connect(&autoPreviewTimer, SIGNAL(timeout()), this,
            SLOT(onPreviewTimerElapsed()));
    connect(checkBoxAutoPreview, SIGNAL(toggled(bool)), this,
            SLOT(onValueChanged()));
    connect(checkBoxDetailedDescription, SIGNAL(toggled(bool)), this,
            SLOT(onDescriptionShow()));
    connect(comboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onSetChanged(int)));
...
void ParameterWidget::onSetDelete() {
    ...
    std::string setName = comboBox->itemData(this->comboBox->currentIndex())
                              .toString()
                              .toStdString();
...
void ParameterWidget::onSetAdd() {
    ...
    updateParameterSet(comboBox->itemData(this->comboBox->currentIndex())
                           .toString()
                           .toStdString());
}
...
void ParameterWidget::readFile(QString scadFile) {
    ...
    if (readonly) {
        ...
        connect(this->deleteButton, SIGNAL(clicked()), this,
                SLOT(onSetDelete()));
    } else {
...
void ParameterWidget::onSetChanged(int idx) {
    const std::string v =
        comboBox->itemData(idx).toString().toUtf8().constData();
    applyParameterSet(v);
    emit previewRequested();
}
...
void ParameterWidget::onPreviewTimerElapsed() { emit previewRequested(); }
...
void ParameterWidget::connectWidget() {
    ...
    for (group_map::iterator it = groupMap.begin(); it != groupMap.end();) {
        ...
        if (gr.empty() || it->first == "Hidden") {
            ...
            it->second.parameterVector.insert(it->second.parameterVector.end(),
                                              global.begin(), global.end());
            it++;
        }
    }
    begin();
    for (std::vector<std::string>::iterator it = groupPos.begin();
         it != groupPos.end(); it++) {
        if (groupMap.find(*it) == groupMap.end()) continue;
        std::vector<std::string> gr;
        ...
        GroupWidget *groupWidget =
            new GroupWidget(groupMap[*it].show, QString::fromStdString(*it));
...
void ParameterWidget::AddParameterWidget(std::string parameterName) {
    ...
    switch (entries[parameterName]->target) {
        case ParameterObject::COMBOBOX: {
            entry =
                new ParameterComboBox(entries[parameterName], descriptionShow);
            break;
        }
        case ParameterObject::SLIDER: {
            entry =
                new ParameterSlider(entries[parameterName], descriptionShow);
            break;
        }
        case ParameterObject::CHECKBOX: {
            entry =
                new ParameterCheckBox(entries[parameterName], descriptionShow);
            break;
        }
        ...
        case ParameterObject::NUMBER: {
            entry =
                new ParameterSpinBox(entries[parameterName], descriptionShow);
            break;
        }
        case ParameterObject::VECTOR: {
            entry =
                new ParameterVector(entries[parameterName], descriptionShow);
            break;
        }
        ...
        if (entry != entries.end()) {
            if (entry->second->dvt == Value::ValueType::STRING) {
                ...
                shared_ptr<Expression> params =
                    CommentParser::parser(v.second.data().c_str());
                if (!params) continue;
...
void ParameterWidget::updateParameterSet(std::string setName) {
    if (setName == "") {
        ...
        bool ok = true;
        const QString result = setDialog->getText(
            this, "Create new set of parameter",
            "Enter name of the parameter set", QLineEdit::Normal, "", &ok);
