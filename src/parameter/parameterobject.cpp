...
int ParameterObject::setValue(const class ValuePtr defaultValue, const class ValuePtr values)
{
	...
	if (dvt == Value::ValueType::BOOL) {
		...
		checkVectorWidget();
	} else if ((vt == Value::ValueType::VECTOR) &&
						 ((dvt == Value::ValueType::NUMBER) || (dvt == Value::ValueType::STRING))) {
		target = COMBOBOX;
...
void ParameterObject::setAssignment(Context *ctx, const Assignment *assignment,
