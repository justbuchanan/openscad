...
Polygon2d *import_svg(const std::string &filename) {
	...
	for (libsvg::shapes_list_t::iterator it = shapes->begin();
	     it != shapes->end(); it++) {
		PRINTD("SVG shape");
		libsvg::shape *s = (*it);
		for (libsvg::path_list_t::iterator it = s->get_path_list().begin();
		     it != s->get_path_list().end(); it++) {
			PRINTD("SVG path");
			libsvg::path_t &p = *it;
			for (libsvg::path_t::iterator it2 = p.begin(); it2 != p.end();
			     it2++) {
