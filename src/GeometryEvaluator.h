...
private:
    ...
    public:
        ...
        ResultObject(shared_ptr<const Geometry> &g)
            : is_const(true), const_pointer(g) {}
        ...
        shared_ptr<const Geometry> constptr() const {
            return is_const ? const_pointer
                            : static_pointer_cast<const Geometry>(pointer);
        }
    ...
    shared_ptr<const Geometry> smartCacheGet(const AbstractNode &node,
                                             bool preferNef);
    bool isSmartCached(const AbstractNode &node);
    std::vector<const class Polygon2d *> collectChildren2D(
    ...
    ResultObject applyToChildren3D(const AbstractNode &node,
                                   OpenSCADOperator op);
