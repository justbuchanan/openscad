...
CGAL_Nef_polyhedron &CGAL_Nef_polyhedron::operator+=(
    const CGAL_Nef_polyhedron &other) {
    (*this->p3) += (*other.p3);
    return *this;
}
...
CGAL_Nef_polyhedron &CGAL_Nef_polyhedron::minkowski(
    const CGAL_Nef_polyhedron &other) {
...
/*!
        Creates a new PolySet and initializes it with the data from this
   polyhedron
...
void CGAL_Nef_polyhedron::resize(const Vector3d &newsize,
    ...
    for (unsigned int i = 0; i < this->getDimension(); i++) {
        if (newsize[i]) {
            if (bbox_size[i] == NT3(0)) {
                PRINT(
                    "WARNING: Resize in direction normal to flat object is not "
                    "implemented");
                return;
            } else {
    ...
    if (newsize[newsizemax_index] != 0) {
        autoscale =
            NT3(newsize[newsizemax_index]) / bbox_size[newsizemax_index];
    }
    for (unsigned int i = 0; i < this->getDimension(); i++) {
...
void CGAL_Nef_polyhedron::transform(const Transform3d &matrix) {
    if (!this->isEmpty()) {
        if (matrix.matrix().determinant() == 0) {
            ...
            CGAL_Aff_transformation t(matrix(0, 0), matrix(0, 1), matrix(0, 2),
