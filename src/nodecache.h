...
/*!
    ...
    The node index guaranteed to be unique per node tree since the index is
   reset every time a new tree is generated.
*/
class NodeCache {
public:
	NodeCache() {}
	 ...
	 *  this reference to be valid for long - if the cache is resized
	 *  internally, existing values are lost. */
	const std::string &insert(const class AbstractNode &node,
